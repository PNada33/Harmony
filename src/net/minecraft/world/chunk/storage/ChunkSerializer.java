...
public class ChunkSerializer
{
    ...
    public static ChunkPrimer read(ServerWorld worldIn, TemplateManager templateManagerIn, PointOfInterestManager poiManager, ChunkPos pos, CompoundNBT compound)
    {
        ...
        ChunkStatus chunkstatus = ChunkStatus.byName(compoundnbt.getString("Status"));
        boolean flag = compoundnbt.contains("isLightOn") && compoundnbt.getInt(STARLIGHT_VERSION_TAG) == STARLIGHT_LIGHT_VERSION;
        ...
        SWMRNibbleArray[] blockNibbles = StarLightEngine.getFilledEmptyLight(worldIn);
        SWMRNibbleArray[] skyNibbles = StarLightEngine.getFilledEmptyLight(worldIn);
        ...
        for (int j = 0; j < listnbt.size(); ++j)
        {
            ...
            if (flag && chunkstatus.isAtLeast(ChunkStatus.LIGHT))
            {
                if (compoundnbt1.contains("BlockLight", 7))
                {
                    blockNibbles[k + 1] = new SWMRNibbleArray(compoundnbt1.getByteArray("BlockLight").clone(), compoundnbt1.getInt(BLOCKLIGHT_STATE_TAG));
                }
                else if (compoundnbt1.contains(BLOCKLIGHT_STATE_TAG, 3))
                {
                    blockNibbles[k + 1] = new SWMRNibbleArray((byte[])null, compoundnbt1.getInt(BLOCKLIGHT_STATE_TAG));
                }

                if (flag1 && compoundnbt1.contains("SkyLight", 7))
                {
                    skyNibbles[k + 1] = new SWMRNibbleArray(compoundnbt1.getByteArray("SkyLight").clone(), compoundnbt1.getInt(SKYLIGHT_STATE_TAG));
                }
                else if (flag1 && compoundnbt1.contains(SKYLIGHT_STATE_TAG, 3))
                {
                    skyNibbles[k + 1] = new SWMRNibbleArray((byte[])null, compoundnbt1.getInt(SKYLIGHT_STATE_TAG));
                }
            }
        }
        ...
        ((ExtendedChunk)ichunk).setBlockNibbles(blockNibbles);
        ((ExtendedChunk)ichunk).setSkyNibbles(skyNibbles);
    ...
    public static CompoundNBT write(ServerWorld worldIn, IChunk chunkIn)
    {
        ...
        SWMRNibbleArray[] blockNibbles = ((ExtendedChunk)chunkIn).getBlockNibbles();
        SWMRNibbleArray[] skyNibbles = ((ExtendedChunk)chunkIn).getSkyNibbles();
        ...
        for (int i = -1; i < 17; ++i)
        {
            ...
            SWMRNibbleArray.SaveState blockNibble = flag && chunkIn.getStatus().isAtLeast(ChunkStatus.LIGHT) ? blockNibbles[j + 1].getSaveState() : null;
            SWMRNibbleArray.SaveState skyNibble = flag && chunkIn.getStatus().isAtLeast(ChunkStatus.LIGHT) ? skyNibbles[j + 1].getSaveState() : null;

            if (chunksection != Chunk.EMPTY_SECTION || blockNibble != null || skyNibble != null)
            {
                ...
                if (blockNibble != null)
                {
                    ...
                    {
                        compoundnbt2.putByteArray("BlockLight", blockNibble.data);
                    }

                    compoundnbt2.putInt(BLOCKLIGHT_STATE_TAG, blockNibble.state);
                }
                ...
                if (skyNibble != null)
                {
                    ...
                    {
                        compoundnbt2.putByteArray("SkyLight", skyNibble.data);
                    }

                    compoundnbt2.putInt(SKYLIGHT_STATE_TAG, skyNibble.state);
                }
        ...
        if (flag)
        {
            compoundnbt1.putBoolean("isLightOn", false);
            compoundnbt1.putInt(STARLIGHT_VERSION_TAG, STARLIGHT_LIGHT_VERSION);
        }
