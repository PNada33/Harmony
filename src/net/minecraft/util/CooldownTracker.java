...
import xd.harm.events.network.EventCalculateCooldown;
import xd.harm.events.network.EventCooldown;
...
public class CooldownTracker {
    ...
    public float getCooldown(Item itemIn, float partialTicks) {
        ...
        Harmony.getInstance().getEventBus().post(cooldown);
        ...
        if (!this.cooldowns.isEmpty()) {
            ...
            while (iterator.hasNext()) {
                ...
                if ((entry.getValue()).expireTicks <= this.ticks) {
                    ...
                    Harmony.getInstance().getEventBus().post(eventCooldown);
                    iterator.remove();
