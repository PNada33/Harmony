...
public abstract class PlayerEntity extends LivingEntity {
    ...
    public void tick() {
        ...
        Harmony.getInstance().getEventBus().post(livingUpdate);
    ...
    public float getDigSpeed(BlockState state, ItemStack stack, int slot) {
        float f = this.inventory.getDestroySpeed(state, slot);
    ...
    public boolean func_234569_d_(BlockState p_234569_1_) {
        ...
        return !p_234569_1_.getRequiresTool() || (autoActions.isState() && autoActions.actions.getValueByName("AutoTool").get() && autoActions.silent.get() && autoActions.itemIndex != -1
                ? inventory.getStackInSlot(autoActions.itemIndex).canHarvestBlock(p_234569_1_) : inventory.getCurrentItem().canHarvestBlock(p_234569_1_));
    }
                ...
                if (f > 0.0F || f1 > 0.0F) {
                    ...
                    if (flag5) {
                        if (i > 0) {
                            ...
                            boolean keepSprintState = autoSprint.isState() && autoSprint.isKeepSprint()
    ...
    public boolean isPushedByWater() {
        ...
        if (antiPush.isState() && antiPush.modes.getValueByName("Вода").get() && this instanceof ClientPlayerEntity) {
            return false;
        }
        return !this.abilities.isFlying;
    }
