...
public class NetworkManager extends SimpleChannelInboundHandler<IPacket<?>> {
    ...
    protected void channelRead0(ChannelHandlerContext p_channelRead0_1_, IPacket<?> p_channelRead0_2_) throws Exception {
        if (this.channel.isOpen()) {
            ...
            Harmony.getInstance().getEventBus().post(e);
    ...
    public static NetworkManager createNetworkManagerAndConnect(InetAddress address, int serverPort, boolean useNativeTransport) {
        String proxyHost = System.getProperty("bot.proxyHost");
        String proxyPortStr = System.getProperty("bot.proxyPort");
        final boolean hasProxy = proxyHost != null && !proxyHost.isEmpty();
        ...
        new Bootstrap().group(lazyvalue.getValue()).handler(new ChannelInitializer<Channel>() {
            ...
            protected void initChannel(Channel channel) {
                ...
                if (hasProxy) {
                    ...
                    channel.pipeline().addLast("socks5", new Socks5ProxyHandler(new InetSocketAddress(proxyHost, proxyPort)));
                }
                ...
                if (channel instanceof SocketChannel) {
                    ...
                    if (ViaLoadingBase.getInstance().getTargetVersion().getVersion() != Harmony.getInstance().getViaMCP().getNATIVE_VERSION()) {
    ...
    public void handleDisconnection() {
        if (this.channel != null && !this.channel.isOpen()) {
            if (this.disconnected) {
                ...
                HarmonyChestTrackerBootstrap.handleDisconnect();
