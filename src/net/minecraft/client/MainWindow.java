...
public final class MainWindow implements AutoCloseable {
    ...
    public int calcGuiScale(int guiScaleIn, boolean forceUnicode) {
        ...
        for (scale = 1; scale != guiScaleIn && scale < this.framebufferWidth && scale < this.framebufferHeight
                && this.framebufferWidth / (scale + 1) >= 320 && this.framebufferHeight / (scale + 1) >= 240; ++scale) {
        }
    ...
    public int height() {
        int scale = 0;
        int i = (int) ((double) this.height / 2);

        scale = (double) this.height / 2 > (double) i ? i + 1 : i;
    ...
    public int width() {
        int scale = 0;
        int i = (int) ((double) this.width / 2);

        scale = (double) this.width / 2 > (double) i ? i + 1 : i;
