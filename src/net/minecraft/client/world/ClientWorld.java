...
public class ClientWorld extends World implements ExtendedWorld {
    ...
    public void removeEntityFromWorld(int eid) {
        ...
        if (entity != null) {
            entity.remove();
            this.removeEntity(entity);
            // Ghost bot entities use very large negative ids and should not trigger "player left" trackers.
            if (eid > -1_000_000_000) {
                eventEntityLeave.setEntity(entity);
                Harmony.getInstance().getEventBus().post(eventEntityLeave);
            }
        }
    }
    ...
    public Chunk getChunkAtImmediately(int chunkX, int chunkZ) {
        return this.field_239129_E_.getChunk(chunkX, chunkZ, ChunkStatus.FULL, false);
    }
