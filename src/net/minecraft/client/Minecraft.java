...
import xd.harm.modules.impl.misc.BotAttack;
import xd.harm.modules.impl.player.ItemCooldown;
...
import net.minecraft.client.WindowStyle;
...
public class Minecraft extends RecursiveEventLoop<Runnable> implements ISnooperInfo, IWindowEventListener {
    ...
    private boolean exitScreenRequested = false;
    @Nullable
    ...
    public Minecraft(GameConfiguration gameConfig) {
        ...
        if (s != null) {
            this.displayGuiScreen(new ConnectingScreen(new MainScreen(), this, s, i));
        } else {
        ...
        this.setLoadingGui(new ResourceLoadProgressGui(this,
    ...
    public void setDefaultMinecraftTitle() {
        ...
        StreamerMode streamerMode = moduleManager.getStreamerMode();

        String inGameUsername = mc.session.getUsername() != null && !mc.session.getUsername().isEmpty() ? mc.session.getUsername() : "Unknown";
        if (streamerMode != null && streamerMode.isState() && streamerMode.options.getValueByName("Ник").get()) {
            inGameUsername = "Protected";
        }
        this.mainWindow.setWindowTitle(Harmony.name + Harmony.version + "(" + inGameUsername + ")");
    }
    ...
    public CompletableFuture<Void> reloadResources() {
        if (this.futureRefreshResources != null) {
            ...
            if (this.loadingGui instanceof ResourceLoadProgressGui) {
                ...
                this.resourcePackRepository.reloadPacksFromFinders();
                List<IResourcePack> list = this.resourcePackRepository.func_232623_f_();
                this.setLoadingGui(new ResourceLoadProgressGui(this, this.resourceManager.reloadResources(
                        Util.getServerExecutor(), this, RESOURCE_RELOAD_INIT_TASK, list), (throwable) -> {
                    Util.acceptOrElse(throwable, this::restoreResourcePacks, () -> {
        ...
        if (!this.isIntegratedServerRunning() && !this.isChatEnabled()) {
            if (this.player != null) {
                this.player.sendMessage(
                        (new TranslationTextComponent("chat.cannotSend")).mergeStyle(TextFormatting.RED),
                        Util.DUMMY_UUID);
            }
        } else {
    ...
    public void displayGuiScreen(@Nullable Screen guiScreenIn) {
        if (this.currentScreen != null) {
            HarmonyChestTrackerBootstrap.handleScreenClose(this.currentScreen);
            this.currentScreen.onClose();
        }

        if (guiScreenIn instanceof net.minecraft.client.gui.screen.MainMenuScreen) {
        ...
        if (guiScreenIn == null && this.world == null) {
            ...
            if (this.player.isShowDeathScreen()) {
                Vector3d deathPos = new Vector3d(this.player.getPosX(), this.player.getPosY(), this.player.getPosZ());
                guiScreenIn = new DeathScreen((ITextComponent) null, this.world.getWorldInfo().isHardcore(), deathPos);
            } else {
                this.player.respawnPlayer();
            }
        }

        if (guiScreenIn instanceof MainScreen || guiScreenIn instanceof MultiplayerScreen) {
        ...
        if (guiScreenIn != null) {
            ...
            guiScreenIn.init(this, HarmonyGuiScale.getScreenWidth(this, guiScreenIn),
                    HarmonyGuiScale.getScreenHeight(this, guiScreenIn));
    ...
    public void shutdownMinecraftApplet() {
        try {
            ...
            HarmonyChestTrackerBootstrap.fireClientStopping(this);
            this.close();
        } finally {
            Util.nanoTimeSupplier = System::nanoTime;

            if (this.crashReporter == null) {
                BotAttack.shutdownAll();
                System.exit(0);
            }
        }
    }
    ...
    private void tick(boolean isDebug, @Nullable LongTickDetector detector) {
        Harmony.getInstance().getEventBus().post(tickEvent);
    ...
    public void updateWindowSize() {
        ...
        if (this.currentScreen != null) {
            this.currentScreen.resize(this, HarmonyGuiScale.getScreenWidth(this, this.currentScreen),
                    HarmonyGuiScale.getScreenHeight(this, this.currentScreen));
        }
    ...
    public void shutdown() {
        if (this.exitScreenRequested) {
            return;
        }
        this.exitScreenRequested = true;
        if (!(this.currentScreen instanceof xd.harm.ui.mainmenu.ExitScreen)) {
            this.displayGuiScreen(new xd.harm.ui.mainmenu.ExitScreen());
        }
    }
                    ...
                    if (this.objectMouseOver != null) {
                        switch (this.objectMouseOver.getType()) {
                            case ENTITY -> {
                                ...
                                boolean hitAuraIsStateAndTargetNotNull = hitAura.isState() && hitAura.getTarget() != null;

                                if (!hitAuraIsStateAndTargetNotNull) {
     ...
     */
    public void runTick() {
        HarmonyChestTrackerBootstrap.fireClientTickStart(this);
        ...
        if (this.currentScreen == null && this.player != null) {
            final BiFunction<EventState, xd.harm.baritone.api.event.events.TickEvent.Type, xd.harm.baritone.api.event.events.TickEvent> tickProvider = xd.harm.baritone.api.event.events.TickEvent.createNextProvider();

            for (IBaritone baritone : BaritoneAPI.getProvider().getAllBaritones()) {

                xd.harm.baritone.api.event.events.TickEvent.Type type = baritone.getPlayerContext().player() != null && baritone.getPlayerContext().world() != null
                        ? xd.harm.baritone.api.event.events.TickEvent.Type.IN
                        : xd.harm.baritone.api.event.events.TickEvent.Type.OUT;
        ...
        if (this.world != null) {
            Harmony.getInstance().getEventBus().post(player.tick);
        ...
        if (this.world != null) {
            if (!this.isGamePaused) {
                if (!this.gameSettings.field_244601_E && this.func_244600_aM()) {
                    ...
                    ITextComponent itextcomponent1 = new TranslationTextComponent(
                            "tutorial.socialInteractions.description",
                            Tutorial.createKeybindComponent("socialInteractions"));
                    this.field_244598_aV = new TutorialToast(TutorialToast.Icons.SOCIAL_INTERACTIONS, itextcomponent,
                            itextcomponent1, true);
            ...
            HarmonyChestTrackerBootstrap.fireWorldTickEnd(this.world);
        ...
        xd.harm.events.world.TickEvent event = new xd.harm.events.world.TickEvent();
        xd.harm.events.EventManager.call(event);
        HarmonyChestTrackerBootstrap.fireClientTickEnd(this);
        this.profiler.endSection();
    }
    ...
    public void loadWorld(String worldName) {
        ...
        this.loadWorld(worldName, DynamicRegistries.func_239770_b_(), Minecraft::loadDataPackCodec,
                Minecraft::loadWorld, false, WorldSelectionType.BACKUP);
    }
    ...
    private void deleteWorld(WorldSelectionType selectionType, String worldName, boolean customized,
                             Runnable runnable) {
    ...
    public void loadWorld(ClientWorld worldClientIn) {
        ...
        Harmony.getInstance().getEventBus().post(new EventChangeWorld());
    }
    ...
    public boolean isEntityGlowing(Entity entity) {
        return entity.isGlowing() || this.player != null && this.player.isSpectator() && this.gameSettings.keyBindSpectatorOutlines.isKeyDown() && entity.getType() == EntityType.PLAYER && !Harmony.getInstance().getModuleManager().getNoRender().element.getValueByName("Эффект свечения").get();
    }
