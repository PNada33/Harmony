...
public abstract class AbstractGui implements AccessorDrawableHelper
{
    ...
    @Override
    public void whereisit$fillGradient(MatrixStack matrixStack, int x1, int y1, int x2, int y2, int colorFrom, int colorTo)
    {
    ...
    public static void blit(MatrixStack matrixStack, float x, float y, float blitOffset, float width, float height, TextureAtlasSprite sprite) {
        innerBlit(matrixStack.getLast().getMatrix(), x, x + width, y, y + height, blitOffset, sprite.getMinU(), sprite.getMaxU(), sprite.getMinV(), sprite.getMaxV());
    }

    public void blit(MatrixStack matrixStack, float x, float y, float uOffset, float vOffset, float uWidth, float vHeight) {
        blit(matrixStack, x, y, this.blitOffset, uOffset, vOffset, uWidth, vHeight, 256, 256);
    }

    public static void blit(MatrixStack matrixStack, float x, float y, float blitOffset, float uOffset, float vOffset, float uWidth, float vHeight, float textureHeight, float textureWidth) {
        innerBlit(matrixStack, x, x + uWidth, y, y + vHeight, blitOffset, uWidth, vHeight, uOffset, vOffset, textureWidth, textureHeight);
    }
