...
public class DebugOverlayGui extends AbstractGui {
    ...
    protected List<String> getDebugInfoLeft() {
        return this.getInfoLeft();
    }

    protected List<String> getInfoLeft() {
        BlockPos blockpos = this.mc.getRenderViewEntity().getPosition();
        String s = "Minecraft " + SharedConstants.getVersion().getName() + " (" + this.mc.getVersion() + "/" + ClientBrandRetriever.getClientModName() + ("release".equalsIgnoreCase(this.mc.getVersionType()) ? "" : "/" + this.mc.getVersionType()) + ")";

        if (this.mc.isReducedDebug()) {
            return Lists.newArrayList(s, this.mc.debug, "", String.format(Locale.ROOT, "XYZ: %.3f / %.5f / %.3f", this.mc.getRenderViewEntity().getPosX(), this.mc.getRenderViewEntity().getPosY(), this.mc.getRenderViewEntity().getPosZ()), String.format("Block: %d %d %d", blockpos.getX(), blockpos.getY(), blockpos.getZ()));
        } else {
            ...
            List<String> list = Lists.newArrayList(s, this.mc.debug, "");
