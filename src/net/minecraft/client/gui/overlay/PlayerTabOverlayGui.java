...
import xd.harm.utils.math.animation.util.Easings;
...
public class PlayerTabOverlayGui extends AbstractGui {
    ...
    public ITextComponent getDisplayName(NetworkPlayerInfo info) {
        Scoreboard scoreboard = this.mc.world != null ? this.mc.world.getScoreboard() : null;
        return getDisplayName(info, scoreboard);
    }

    public ITextComponent getDisplayName(NetworkPlayerInfo info, @Nullable Scoreboard scoreboard) {
        ScorePlayerTeam team = scoreboard != null ? scoreboard.getPlayersTeam(info.getGameProfile().getName()) : info.getPlayerTeam();
        IFormattableTextComponent component = info.getDisplayName() != null
                ? info.getDisplayName().deepCopy()
                : ScorePlayerTeam.formatPlayerName(team, new StringTextComponent(info.getGameProfile().getName()));
        return info.getGameType() == GameType.SPECTATOR ? component.mergeStyle(TextFormatting.ITALIC) : component;
    }
    ...
    public void setVisible(boolean visible) {
        ...
        animation.run(visible ? 1 : 0, 0.3, visible ? Easings.BACK_OUT : Easings.BACK_IN, true);
    }
    ...
    public void func_238523_a_(MatrixStack p_238523_1_, int p_238523_2_, Scoreboard p_238523_3_, @Nullable ScoreObjective p_238523_4_) {
        ...
        List<NetworkPlayerInfo> controlledEntries = botSessionManager.getRenderTabEntries();
        boolean useControlledEntries = controlledEntries != null;
        ClientPlayNetHandler clientplaynethandler = this.mc.player.connection;
        List<NetworkPlayerInfo> list = new ArrayList<>();

        boolean isHolytimeServer = mc.getCurrentServerData() != null && mc.getCurrentServerData().serverIP.contains("holytime.su");

        for (ScorePlayerTeam team : p_238523_3_.getTeams().stream().sorted(Comparator.comparing(Team::getName)).toList()) {
            ...
            if (namePattern.matcher(name).matches() && !team.getPrefix().getString().isEmpty()) {
                ...
                List<NetworkPlayerInfo> knownPlayers = useControlledEntries ? controlledEntries : new ArrayList<>(mc.getConnection().getPlayerInfoMap());
                for (NetworkPlayerInfo info : knownPlayers) {
                ...
                if (vanish) {
                    IFormattableTextComponent vanishedText = (IFormattableTextComponent) team.getPrefix().deepCopy();
                    vanishedText.appendString(name);

                    GameProfile fakeProfile = new GameProfile(UUID.nameUUIDFromBytes(name.getBytes()), name);
                    list.add(new NetworkPlayerInfo(new SPlayerListItemPacket.AddPlayerData(
                            fakeProfile, 0, GameType.SURVIVAL, vanishedText)));
                }
            }
        }
        ...
        if (useControlledEntries) {
            list.addAll(ENTRY_ORDERING.sortedCopy(controlledEntries));
        } else {
        ...
        for (NetworkPlayerInfo networkplayerinfo : list) {
            int k = this.mc.fontRenderer.getStringPropertyWidth(this.getDisplayName(networkplayerinfo, p_238523_3_));
            i = Math.max(i, k);
        ...
        boolean isBetter = visuality.isState() && visuality.betterTab.get();
        ...
        for (int i5 = 0; i5 < i4; ++i5) {
            ...
            if (i5 < list.size()) {
                ...
                boolean isVanished = !mc.getConnection().getPlayerInfoMap().contains(networkplayerinfo1);
                boolean isMe = networkplayerinfo1.getGameProfile().getName().equals(mc.player.getGameProfile().getName());
                boolean isSpectator = networkplayerinfo1.getGameType() == GameType.SPECTATOR;

                int backgroundColor = l4;
                if (isVanished) {
                    backgroundColor = 0x40FF0000;
                } else if (isMe) {
                    backgroundColor = 0x4000FFFF;
                } else if (isSpectator) {
                    backgroundColor = 0x40FFFF00;
                }

                fill(p_238523_1_, k2, l2, k2 + i1, l2 + 8, backgroundColor);
            } else {
            ...
            if (i5 < list.size()) {
                ...
                ITextComponent name = getDisplayName(networkplayerinfo1, p_238523_3_);
