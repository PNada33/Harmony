...
public class NewChatGui extends AbstractGui
{
    ...
    public void func_238492_a_(MatrixStack p_238492_1_, int p_238492_2_)
    {
        ...
        if (!this.func_238496_i_())
        {
            ...
            if (k > 0)
            {
                ...
                float hotbarOffset = 0.0F;
                if (ChatScreen.correctPosition) {
                    hotbarOffset = ChatScreen.getHotbarAnimationOutput() * 15.0F;
                }
                RenderSystem.translatef(0.0F, -hotbarOffset, 0.0F);
                ...
                for (int j1 = 0; j1 + this.scrollPos < this.drawnChatLines.size() && j1 < j; ++j1) {
                    ...
                    Visuality visuality = Harmony.getInstance().getModuleManager().getVisuality();
                    boolean isBetter = visuality.isState() && visuality.betterChat.get();
                    if (isBetter) {
                        l = (int) (MathHelper.ceil(mc.fontRenderer.func_243245_a(chatline.getLineString()) / d0) * chatline.getSlideAnimation().getValue());
                    }
                    if (chatline == null || (k1 = p_238492_2_ - chatline.getUpdatedCounter()) >= 200 && !flag) continue;
                    double d5 = flag ? 1.0 : NewChatGui.getLineBrightness(k1);
                    ...
                    if (this.mc.gameSettings.ofChatBackground != 3) {
                        NewChatGui.fill(p_238492_1_, -2, (int)(d6 - d3), l + 4, (int)d6, j2 << 24);
                    }
                    if (Harmony.getInstance().getModuleManager().getVisuality().smoothChat.get()) {
                        chatline.getSlideAnimation().run(1.0);
                    } else {
                        chatline.getSlideAnimation().setValue(1);
                    }
                    p_238492_1_.translate((double)(-NewChatGui.calculateChatboxWidth(this.mc.gameSettings.chatWidth)) + (double)NewChatGui.calculateChatboxWidth(this.mc.gameSettings.chatWidth) * chatline.getSlideAnimation().getValue(), 0.0, 50.0);
                    ...
                    if (!this.mc.gameSettings.ofChatShadow) {
                        this.mc.fontRenderer.func_238422_b_(p_238492_1_, chatline.getLineString(), 0.0f, (int)(d6 + d4), 0xFFFFFF + (i2 << 24));
                    } else {
                        this.mc.fontRenderer.func_238407_a_(p_238492_1_, chatline.getLineString(), 0.0f, (int)(d6 + d4), 0xFFFFFF + (i2 << 24));
                    }
    ...
    public void clearClientChatMessages(boolean clearSentMsgHistory)
    {
        this.field_238489_i_.clear();
        for (ChatLine<ITextComponent> c : chatLines) {
            if (c.isClient()) {
                chatLines.remove(c);
            }
        }
        for (ChatLine<IReorderingProcessor> c : drawnChatLines) {
            if (c.isClient()) {
                chatLines.remove(c);
            }
        }
    ...
    public void printClient(ITextComponent chatComponent, int chatLineId)
    {
        this.func_238493_a_Client(chatComponent, chatLineId, this.mc.ingameGUI.getTicks(), false);
        LOGGER.info("[CHAT] {}", (Object)chatComponent.getString().replaceAll("\r", "\\\\r").replaceAll("\n", "\\\\n"));
    }
    ...
    private void func_238493_a_(ITextComponent p_238493_1_, int p_238493_2_, int p_238493_3_, boolean p_238493_4_)
    {
        ...
        for (IReorderingProcessor ireorderingprocessor : list)
        {
            ...
            this.drawnChatLines.add(0, new ChatLine<>(p_238493_3_, ireorderingprocessor, p_238493_2_, false));
        }
        ...
        if (!p_238493_4_)
        {
            this.chatLines.add(0, new ChatLine<>(p_238493_3_, p_238493_1_, p_238493_2_, false));
    ...
    private void func_238493_a_Client(ITextComponent p_238493_1_, int p_238493_2_, int p_238493_3_, boolean p_238493_4_)
    {
        if (p_238493_2_ != 0)
        {
            this.deleteChatLine(p_238493_2_);
        ...
        for (IReorderingProcessor ireorderingprocessor : list)
        {
            ...
            this.drawnChatLines.add(0, new ChatLine<>(p_238493_3_, ireorderingprocessor, p_238493_2_, true));
        }
        ...
        if (!p_238493_4_)
        {
            this.chatLines.add(0, new ChatLine<>(p_238493_3_, p_238493_1_, p_238493_2_, true));
    ...
    public boolean func_238491_a_(double p_238491_1_, double p_238491_3_)
    {
        if (this.getChatOpen() && !this.mc.gameSettings.hideGUI && !this.func_238496_i_() && !this.field_238489_i_.isEmpty())
        {
            ...
            double d1 = (double)this.mc.getMainWindow().getScaledHeight() - p_238491_3_ - 40.0D - (double)hotbarOffset;
    ...
    public Style func_238494_b_(double p_238494_1_, double p_238494_3_)
    {
        if (this.getChatOpen() && !this.mc.gameSettings.hideGUI && !this.func_238496_i_())
        {
            ...
            double d1 = (double)this.mc.getMainWindow().getScaledHeight() - p_238494_3_ - 40.0D - (double)hotbarOffset;
    ...
    {
        this.drawnChatLines.removeIf((chatLine) -> chatLine.getLineString().equals(filter));
        this.chatLines.removeIf((chatLine) -> chatLine.getLineString().equals(filter));
    }
