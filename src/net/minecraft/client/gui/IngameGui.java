...
import xd.harm.modules.impl.misc.InventoryPlus;
import xd.harm.modules.impl.misc.PlayerHelper;
import xd.harm.modules.impl.render.Crosshair;
...
public class IngameGui extends AbstractGui {
    ...
    private float getChatHudYOffset() {
        if (this.mc.currentScreen instanceof ChatScreen) {
            return -15.0F * ChatScreen.getHotbarAnimationOutput();
        }
    ...
    private int getChatHudYOffsetRounded() {
        return Math.round(this.getChatHudYOffset());
    }
    ...
    EventDisplay eventDisplay = new EventDisplay(new MatrixStack(), 1);

    public void renderIngameGui(MatrixStack matrixStack, float partialTicks) {
        eventDisplay.setMatrixStack(matrixStack);
        eventDisplay.setPartialTicks(partialTicks);
        mc.gameRenderer.setupOverlayRendering(2);
        eventDisplay.setType(EventDisplay.Type.PRE);
        if (eventDisplay.getType() == EventDisplay.Type.PRE && !Boolean.getBoolean("bot.mode")) {
            Harmony.getInstance().getEventBus().post(eventDisplay);
        }
        ...
        if (this.mc.gameSettings.getPointOfView().func_243192_a()
                && itemstack.getItem() == Blocks.CARVED_PUMPKIN.asItem()) {
        ...
        if (!this.mc.gameSettings.hideGUI) {
            if (this.overlayMessage != null && this.overlayMessageTime > 0) {
                ...
                if (i1 > 8) {
                    ...
                    RenderSystem.translatef((float) (this.scaledWidth / 2), (float) (this.scaledHeight - 68 + this.getChatHudYOffset()), 0.0F);
                    RenderSystem.enableBlend();
                    RenderSystem.defaultBlendFunc();
                    int k1 = 16777215;
            ...
            if (eventCancelOverlay.isCancel()) {
                ...
                if (j1 > 8) {
                    ...
                    fontrenderer.func_243246_a(matrixStack, this.displayedTitle, (float) (-i2 / 2), -10.0F,
                            16777215 | l1);
                    RenderSystem.popMatrix();
                    ...
                    if (this.displayedSubTitle != null) {
                        ...
                        fontrenderer.func_243246_a(matrixStack, this.displayedSubTitle, (float) (-k2 / 2), 5.0F,
                                16777215 | l1);
                        RenderSystem.popMatrix();
                    }
            ...
            Scoreboard scoreboard = botSessionManager.getRenderScoreboard(this.mc.world.getScoreboard());
            ...
            RenderSystem.translatef(0.0F, (float) (this.scaledHeight - 48 + this.getChatHudYOffset()), 0.0F);
            ...
            int tabSize = botSessionManager.getRenderTabSize(this.mc.player.connection.getPlayerInfoMap().size());
            if (this.mc.gameSettings.keyBindPlayerList.isKeyDown() && (!this.mc.isIntegratedServerRunning()
                    || tabSize > 1 || scoreobjective1 != null)) {
        ...
        eventDisplay.setMatrixStack(matrixStack);
        eventDisplay.setPartialTicks(partialTicks);
        if (Notify.NOTIFICATION_MANAGER != null) {
            Notify.NOTIFICATION_MANAGER.draw(matrixStack);
        }
        mc.gameRenderer.setupOverlayRendering(2);
        eventDisplay.setType(EventDisplay.Type.POST);
        if (eventDisplay.getType() == EventDisplay.Type.POST && !Boolean.getBoolean("bot.mode")) {
            Harmony.getInstance().getEventBus().post(eventDisplay);
        }
        mc.gameRenderer.setupOverlayRendering(2);
        eventDisplay.setType(EventDisplay.Type.HIGH);
        if (eventDisplay.getType() == EventDisplay.Type.HIGH && !Boolean.getBoolean("bot.mode")) {
            Harmony.getInstance().getEventBus().post(eventDisplay);
        }
        this.renderTabOverlayOnTop(matrixStack);
        if (!Boolean.getBoolean("bot.mode")) {
            HarmonyVoicechatBootstrap.fireRenderHud(matrixStack, partialTicks);
        }
        mc.gameRenderer.setupOverlayRendering();
    ...
    private void renderTabOverlayOnTop(MatrixStack matrixStack) {
        if (this.mc.gameSettings.hideGUI || this.mc.world == null || this.mc.player == null) {
            return;
        }
        ...
        boolean tabKeyDown = this.mc.gameSettings.keyBindPlayerList.isKeyDown();
        int tabSize = botSessionManager.getRenderTabSize(this.mc.player.connection.getPlayerInfoMap().size());
        boolean canRenderTab = !this.mc.isIntegratedServerRunning() || tabSize > 1 || scoreobjective != null;
        boolean tabAnimating = this.overlayPlayerList.getAnimation().getValue() > 0.01;

        if (!tabAnimating && !(tabKeyDown && canRenderTab)) {
            return;
        }
    ...
    protected void renderHotbar(float partialTicks, MatrixStack matrixStack) {
        ...
        if (playerentity != null) {
            int hotbarYOffset = this.getChatHudYOffsetRounded();
            RenderSystem.color4f(1.0F, 1.0F, 1.0F, 1.0F);
            ...
            this.blit(matrixStack, i - 91, this.scaledHeight - 22 + hotbarYOffset, 0, 0, 182, 22);
            this.blit(matrixStack, i - 91 - 1 + playerentity.inventory.currentItem * 20, this.scaledHeight - 22 - 1 + hotbarYOffset, 0,
                    22, 24, 22);

            if (!itemstack.isEmpty()) {
                if (handside == HandSide.LEFT) {
                    this.blit(matrixStack, i - 91 - 29, this.scaledHeight - 23 + hotbarYOffset, 24, 22, 29, 24);
                } else {
                    this.blit(matrixStack, i + 91, this.scaledHeight - 23 + hotbarYOffset, 53, 22, 29, 24);
                }
            }
            ...
            ModuleManager moduleManager = Harmony.getInstance().getModuleManager();
            InventoryPlus inventoryPlus = moduleManager.getInventoryPlus();

            for (int i1 = 0; i1 < 9; ++i1) {
                int j1 = i - 90 + i1 * 20 + 2;
                int k1 = this.scaledHeight - 16 - 3 + hotbarYOffset;
                ItemStack hotbarStack = playerentity.inventory.mainInventory.get(i1);

                if (inventoryPlus.isState() && inventoryPlus.itemHelper.get() && !hotbarStack.isEmpty()) {
                    if (inventoryPlus.shouldHighlight(hotbarStack)) {
                        int highlightColor = inventoryPlus.getHighlightColor(hotbarStack);
                        float alpha = inventoryPlus.getAnimationAlpha();

                        int red = (highlightColor >> 16) & 0xFF;
                        int green = (highlightColor >> 8) & 0xFF;
                        int blue = highlightColor & 0xFF;
                        int animatedAlpha = (int)(255 * alpha);

                        int animatedColor = ColorUtils.rgba(red, green, blue, animatedAlpha);
                        ...
                        fill(matrixStack, j1, k1, j1 + 16, k1 + 16, animatedColor);
                ...
                this.renderHotbarItem(j1, k1, partialTicks, playerentity, hotbarStack);
            }

            if (!itemstack.isEmpty()) {
                CustomItems.setRenderOffHand(true);
                int i2 = this.scaledHeight - 16 - 3 + hotbarYOffset;
                int offhandX = 0;
                ...
                if (inventoryPlus.isState() && inventoryPlus.itemHelper.get()) {
                    if (inventoryPlus.shouldHighlight(itemstack)) {
                        int highlightColor = inventoryPlus.getHighlightColor(itemstack);
                        float alpha = inventoryPlus.getAnimationAlpha();
                        ...
                        fill(matrixStack, offhandX, i2, offhandX + 16, i2 + 16, animatedColor);
            ...
            if (this.mc.gameSettings.attackIndicator == AttackIndicatorStatus.HOTBAR) {
                ...
                if (f < 1.0F) {
                    int j2 = this.scaledHeight - 20 + hotbarYOffset;
                    int k2 = i + 91 + 6;
    ...
    public void func_238457_e_(MatrixStack matrixStack) {
        PlayerEntity playerentity = this.getRenderViewPlayer();

        if (playerentity == null) return;

        PlayerHelper playerHelper = Harmony.getInstance().getModuleManager().getPlayerHelper();
        boolean hungryBarEnabled = playerHelper != null && playerHelper.isState() && playerHelper.isHungryBarEnabled();
        ...
        int saturationFoodLevel = Math.min(20, Math.max(0, MathHelper.ceil(foodstats.getSaturationLevel())));
        boolean renderHungryBar = hungryBarEnabled && l >= 20 && saturationFoodLevel > 0;
        int i1 = this.scaledWidth / 2 - 91;
        int j1 = this.scaledWidth / 2 + 91;
        int k1 = this.scaledHeight - 39 + this.getChatHudYOffsetRounded();
        ...
        for (int l5 = MathHelper.ceil((f + (float) l1) / 2.0F) - 1; l5 >= 0; --l5) {
            ...
            this.blit(matrixStack, l4, i5, 16 + j4 * 9, 9 * j5, 9, 9);

            if (flag) {
                if (l5 * 2 + 1 < k) {
                    this.blit(matrixStack, l4, i5, i6 + 54, 9 * j5, 9, 9);
                }

                if (l5 * 2 + 1 == k) {
                    this.blit(matrixStack, l4, i5, i6 + 63, 9 * j5, 9, 9);
                }
            }

            if (i3 > 0) {
                if (i3 == l1 && l1 % 2 == 1) {
                    this.blit(matrixStack, l4, i5, i6 + 153, 9 * j5, 9, 9);
                    --i3;
                } else {
                    this.blit(matrixStack, l4, i5, i6 + 144, 9 * j5, 9, 9);
                    i3 -= 2;
                }
            } else {
                if (l5 * 2 + 1 < i) {
                    this.blit(matrixStack, l4, i5, i6 + 36, 9 * j5, 9, 9);
                }

                if (l5 * 2 + 1 == i) {
                    this.blit(matrixStack, l4, i5, i6 + 45, 9 * j5, 9, 9);
                }
            }
        }
        ...
        if (j6 == 0) {
            ...
            for (int k6 = 0; k6 < 10; ++k6) {
                ...
                int k8 = j1 - k6 * 8 - 9;
                this.blit(matrixStack, k8, i7, 16 + i8 * 9, 27, 9, 9);

                if (k6 * 2 + 1 < l) {
                    this.blit(matrixStack, k8, i7, k7 + 36, 27, 9, 9);
                }

                if (k6 * 2 + 1 == l) {
                    this.blit(matrixStack, k8, i7, k7 + 45, 27, 9, 9);
                }

                if (renderHungryBar) {
                    int secondRowY = i7 - 10;
                    this.blit(matrixStack, k8, secondRowY, 16 + i8 * 9, 27, 9, 9);

                    if (k6 * 2 + 1 < saturationFoodLevel) {
                        this.blit(matrixStack, k8, secondRowY, k7 + 36, 27, 9, 9);
                    }

                    if (k6 * 2 + 1 == saturationFoodLevel) {
                        this.blit(matrixStack, k8, secondRowY, k7 + 45, 27, 9, 9);
                    }
                }
            }
        ...
        if (playerentity.areEyesInFluid(FluidTags.WATER) || j7 < l6) {
            ...
            for (int k5 = 0; k5 < j8 + l8; ++k5) {
                if (k5 < j8) {
                    this.blit(matrixStack, j1 - k5 * 8 - 9, l2, 16, 18, 9, 9);
                } else {
                    this.blit(matrixStack, j1 - k5 * 8 - 9, l2, 25, 18, 9, 9);
                }
            }
        }
    ...
    public void func_238454_b_(MatrixStack matrixStack, int xPosition) {
        ...
        if (i > 0) {
            ...
            int l = this.scaledHeight - 32 + 3 + this.getChatHudYOffsetRounded();
            this.blit(matrixStack, xPosition, l, 0, 64, 182, 5);

            if (k > 0) {
                this.blit(matrixStack, xPosition, l, 0, 69, k, 5);
            }
        }
        ...
        if (this.mc.player.experienceLevel > 0) {
            ...
            int i1 = this.scaledHeight - 31 - 4 + this.getChatHudYOffsetRounded();
            this.getFontRenderer().drawString(matrixStack, s, (float) (k1 + 1), (float) i1, 0);
            this.getFontRenderer().drawString(matrixStack, s, (float) (k1 - 1), (float) i1, 0);
            this.getFontRenderer().drawString(matrixStack, s, (float) k1, (float) (i1 + 1), 0);
            this.getFontRenderer().drawString(matrixStack, s, (float) k1, (float) (i1 - 1), 0);
            this.getFontRenderer().drawString(matrixStack, s, (float) k1, (float) i1, j1);
            this.mc.getProfiler().endSection();
        }
    }

    private void func_238448_a_(MatrixStack p_238448_1_, FontRenderer p_238448_2_, int p_238448_3_, int p_238448_4_,
                                int p_238448_5_) {
        int i = this.mc.gameSettings.getTextBackgroundColor(0.0F);

        if (i != 0) {
            int j = -p_238448_4_ / 2;
            fill(p_238448_1_, j - 2, p_238448_3_ - 2, j + p_238448_4_ + 2, p_238448_3_ + 9 + 2,
                    ColorHelper.PackedColor.blendColors(i, p_238448_5_));
        }
    }
    ...
    private void func_238456_d_(MatrixStack p_238456_1_) {
        ...
        if (gamesettings.getPointOfView().func_243192_a()
                && (this.mc.playerController.getCurrentGameType() != GameType.SPECTATOR
                || this.isTargetNamedMenuProvider(this.mc.objectMouseOver))) {
            if (gamesettings.showDebugInfo && !gamesettings.hideGUI && !this.mc.player.hasReducedDebug()
                    && !gamesettings.reducedDebugInfo) {
                RenderSystem.pushMatrix();
                RenderSystem.translatef((float) (this.scaledWidth / 2), (float) (this.scaledHeight / 2),
                        (float) this.getBlitOffset());
                ...
                int i = 15;
                this.blit(p_238456_1_, (this.scaledWidth - 15) / 2, (this.scaledHeight - 15) / 2, 0, 0, 15, 16);
            ...
            while (true) {
                ...
                if (effectinstance.isShowIcon()) {
                    ...
                    if (Reflector.IForgeEffectInstance_renderHUDEffect.exists()) {
                        Reflector.call(effectinstance, Reflector.IForgeEffectInstance_renderHUDEffect, this,
                                matrixStack, k, l, this.getBlitOffset(), f);
                    }
                }
            }
        }
    ...
    public void renderHorseJumpBar(MatrixStack matrixStack, int xPosition) {
        ...
        int i = 182;
        int j = (int) (f * 183.0F);
        int k = this.scaledHeight - 32 + 3 + this.getChatHudYOffsetRounded();
    ...
    public void func_238453_b_(MatrixStack p_238453_1_) {
        ...
        if (this.remainingHighlightTicks > 0 && !this.highlightingItemStack.isEmpty()) {
            IFormattableTextComponent iformattabletextcomponent = (new StringTextComponent(""))
                    .append(this.highlightingItemStack.getDisplayName())
                    .mergeStyle(this.highlightingItemStack.getRarity().color);
            ...
            if (Reflector.IForgeItemStack_getHighlightTip.exists()) {
                itextcomponent = (ITextComponent) Reflector.call(this.highlightingItemStack,
                        Reflector.IForgeItemStack_getHighlightTip, iformattabletextcomponent);
            }

            String plainText = itextcomponent.getString();

            float fontSize = 7f;
            float textWidth = Fonts.sfui.getWidth(plainText, fontSize);
            float textHeight = Fonts.sfui.getHeight(fontSize);

            float x = (this.scaledWidth - textWidth) / 2f;

            float y = this.scaledHeight - 64 + this.getChatHudYOffset();
            ...
            if (l > 0) {
                ...
                float bgX = x - 2;
                float bgY = y - 4;
                float bgW = textWidth + 4;
                float bgH = textHeight + 4;

                GaussianBlur.startBlur();
                RenderUtility.drawRoundedRect(bgX, bgY, bgW, bgH,
                        new Vector4f(3, 3, 3, 3), -1);
                GaussianBlur.endBlur(3f, 1f);

                int bgAlpha = (int)(l * 0.5f);
                int bgColor = ColorUtils.rgba(16, 16, 16, bgAlpha);

                RenderUtility.drawRoundedRect(bgX, bgY, bgW, bgH,
                        new Vector4f(3, 3, 3, 3), bgColor);

                int textColor;
                Rarity rarity = this.highlightingItemStack.getRarity();

                if (rarity == Rarity.UNCOMMON) {
                    textColor = ColorUtils.rgba(255, 255, 85, l);
                } else if (rarity == Rarity.RARE) {
                    textColor = ColorUtils.rgba(85, 255, 255, l);
                } else if (rarity == Rarity.EPIC) {
                    textColor = ColorUtils.rgba(255, 85, 255, l);
                } else {
                    textColor = ColorUtils.rgba(255, 255, 255, l);
                }

                if (this.highlightingItemStack.hasDisplayName()) {
                    int themeColor = Theme.MainColor(0);
                    textColor = ColorUtils.interpolateColor(textColor, themeColor, 0.15f);
                    textColor = ColorUtils.setAlpha(textColor, l);
                }

                Fonts.sfui.drawText(p_238453_1_, plainText, x, y - 2, textColor, fontSize);
    ...
    public void func_238455_c_(MatrixStack p_238455_1_) {
        ...
        if (this.mc.world.getGameTime() >= 120500L) {
            ...
            itextcomponent = new TranslationTextComponent("demo.remainingTime",
                    StringUtils.ticksToElapsedTime((int) (120500L - this.mc.world.getGameTime())));
        }

        int i = this.getFontRenderer().getStringPropertyWidth(itextcomponent);
        this.getFontRenderer().func_243246_a(p_238455_1_, itextcomponent, (float) (this.scaledWidth - i - 10), 5.0F,
                16777215);
        this.mc.getProfiler().endSection();
    }
    ...
    private void func_238447_a_(MatrixStack p_238447_1_, ScoreObjective p_238447_2_) {
        ...
        RenderUtility.drawRoundedRect(l2 - 2, j2 - i2, scaledWidth - l2, i2, new Vector4f(3, 0, 3, 0), ColorUtils.rgba(0, 0, 0, 80));
        ...
        for (Pair<Score, ITextComponent> pair : list1) {
            ...
            if (l == collection.size()) {
                RenderUtility.drawRoundedRect(l2 - 2, k1 - 8.7f, scaledWidth - l2, 9f, new Vector4f(0.5f, 4, 0, 4), ColorUtils.rgba(0, 0, 0, 128));
    ...
    private void func_238458_f_(MatrixStack p_238458_1_) {
        ...
        if (livingentity != null) {
            ...
            if (i != 0) {
                ...
                int k = this.scaledHeight - 39 + this.getChatHudYOffsetRounded();
    ...
    private void updateVignetteBrightness(Entity entityIn) {
        if (entityIn != null) {
            ...
            this.prevVignetteBrightness = (float) ((double) this.prevVignetteBrightness
                    + (double) (f - this.prevVignetteBrightness) * 0.01D);
        }
    }
    ...
    private void renderVignette(Entity entityIn) {
        if (!Config.isVignetteEnabled()) {
            ...
            if (f > 0.0F) {
                ...
                RenderSystem.color4f(this.prevVignetteBrightness, this.prevVignetteBrightness,
            ...
            bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX);
            bufferbuilder.pos(0.0D, (double) this.scaledHeight, -90.0D).tex(0.0F, 1.0F).endVertex();
            bufferbuilder.pos((double) this.scaledWidth, (double) this.scaledHeight, -90.0D).tex(1.0F, 1.0F)
                    .endVertex();
            bufferbuilder.pos((double) this.scaledWidth, 0.0D, -90.0D).tex(1.0F, 0.0F).endVertex();
            bufferbuilder.pos(0.0D, 0.0D, -90.0D).tex(0.0F, 0.0F).endVertex();
            tessellator.draw();
    ...
    public void tick() {
        ...
        if (ChatScreen.hotbarAnimation != null && ChatScreen.hotbarAnimation.getDirection() == Direction.BACKWARDS) {
            ChatScreen.hotbarAnimation.isDone();
        }
        ...
        if (this.mc.player != null) {
            ...
            if (Reflector.IForgeItemStack_getHighlightTip.exists()) {
                ITextComponent itextcomponent = (ITextComponent) Reflector.call(itemstack,
                        Reflector.IForgeItemStack_getHighlightTip, itemstack.getDisplayName());
                ITextComponent itextcomponent1 = (ITextComponent) Reflector.call(this.highlightingItemStack,
                        Reflector.IForgeItemStack_getHighlightTip, this.highlightingItemStack.getDisplayName());
            ...
            if (itemstack.isEmpty()) {
                this.remainingHighlightTicks = 0;
                    ...
                    && itemstack.getDisplayName().equals(this.highlightingItemStack.getDisplayName()) && flag) {
    ...
    public void func_238452_a_(@Nullable ITextComponent p_238452_1_, @Nullable ITextComponent p_238452_2_,
                               int p_238452_3_, int p_238452_4_, int p_238452_5_) {
    ...
    public void func_238450_a_(ChatType p_238450_1_, ITextComponent p_238450_2_, UUID p_238450_3_) {
        if (!this.mc.cannotSendChatMessages(p_238450_3_) && (!this.mc.gameSettings.field_244794_ae
                || !this.mc.cannotSendChatMessages(this.func_244795_b(p_238450_2_)))) {
