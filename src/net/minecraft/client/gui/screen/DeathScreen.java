...
public class DeathScreen extends Screen implements IMinecraft {
    ...
    private final Vector3d deathPosition;

    public DeathScreen(@Nullable ITextComponent textComponent, boolean isHardcoreMode, Vector3d deathPosition) {
    ...
    @Override
    protected void init() {
        int x = (int) deathPosition.x;
        int y = (int) deathPosition.y;
        int z = (int) deathPosition.z;

        Harmony.getInstance().getModuleManager().getPlayerHelper();
        PlayerHelper playerHelper = Harmony.getInstance().getModuleManager().getPlayerHelper();
        if (playerHelper != null && playerHelper.deathPosition.get() && mc.player.deathTime < 0.1) {
