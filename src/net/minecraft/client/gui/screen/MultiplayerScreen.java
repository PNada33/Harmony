...
public class MultiplayerScreen extends Screen {
    ...
    private final VersionSelectScreen viaScreen = Harmony.getInstance().getViaMCP().getViaScreen();
    ...
    protected void init() {
        ...
        addButton(Harmony.getInstance().getViaMCP().viaScreen);
    ...
    public void tick() {
        super.tick();
        Harmony.getInstance().getViaMCP().viaScreen.tick();
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        this.renderServerListBackdrop(matrixStack);
        this.serverListSelector.render(matrixStack, mouseX, mouseY, partialTicks);
        this.renderServerListOutline(matrixStack);
        this.renderControlPanel(matrixStack);
    ...
    private void renderServerListBackdrop(MatrixStack matrixStack) {
        float panelX = this.serverListSelector.getRowLeft() - 10.0f;
        float panelY = 28.0f;
        float panelWidth = this.serverListSelector.getRowWidth() + 20.0f;
        float panelHeight = this.height - 94.0f;
        float radius = 10.0f;

        GlStateManager.enableBlend();
        GlStateManager.blendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
        RenderUtility.drawShadow(panelX - 3.0f, panelY - 3.0f, panelWidth + 6.0f, panelHeight + 6.0f, 20, ColorUtils.rgba(0, 0, 0, 60));
        RenderUtility.drawRoundedRect(panelX, panelY, panelWidth, panelHeight, new Vector4f(radius, radius, radius, radius), ColorUtils.rgba(0, 0, 0, 48));
        GlStateManager.disableBlend();
    }

    private void renderServerListOutline(MatrixStack matrixStack) {
        float panelX = this.serverListSelector.getRowLeft() - 10.0f;
        float panelY = 28.0f;
        float panelWidth = this.serverListSelector.getRowWidth() + 20.0f;
        float panelHeight = this.height - 94.0f;
        ...
        GlStateManager.enableBlend();
        GlStateManager.blendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
        RenderUtility.drawRoundedRectOutline(panelX, panelY, panelWidth, panelHeight, radius, 1.0f, ColorUtils.rgba(0, 0, 0, 170));
        RenderUtility.drawRoundedRectOutline(panelX + 1.0f, panelY + 1.0f, panelWidth - 2.0f, panelHeight - 2.0f, radius - 1.0f, 0.8f, ColorUtils.setAlpha(ColorUtils.brighter(accent, 0.15f), 95));
        GlStateManager.disableBlend();
    }

    private void renderControlPanel(MatrixStack matrixStack) {
        float panelWidth = 328.0f;
        float panelHeight = 54.0f;
        float panelX = this.width / 2.0f - panelWidth / 2.0f;
        float panelY = this.height - 58.0f;
        float radius = 9.0f;

        GlStateManager.enableBlend();
        GlStateManager.blendFunc(GL11.GL_SRC_ALPHA, GL11.GL_ONE_MINUS_SRC_ALPHA);
        RenderUtility.drawShadow(panelX - 2.0f, panelY - 2.0f, panelWidth + 4.0f, panelHeight + 4.0f, 18, ColorUtils.rgba(0, 0, 0, 70));
        RenderUtility.drawRoundedRect(panelX, panelY, panelWidth, panelHeight, new Vector4f(radius, radius, radius, radius), ColorUtils.rgba(0, 0, 0, 90));
        RenderUtility.drawRoundedRectOutline(panelX, panelY, panelWidth, panelHeight, radius, 1.1f, ColorUtils.rgba(0, 0, 0, 170));
        GlStateManager.disableBlend();
    }
