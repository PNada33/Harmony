...
public class InventoryScreen extends DisplayEffectsScreen<PlayerContainer> implements IRecipeShownListener, IMinecraft {
    ...
    private boolean isDroppingItems = false;
    private int currentDropIndex = 0;
    private long lastDropTime = 0;
    private static final long DROP_DELAY = 1;
    ...
    private void updateItemDropping() {
        if (isDroppingItems && mc.currentScreen == this) {
            long currentTime = System.currentTimeMillis();
            if (currentTime - lastDropTime >= DROP_DELAY) {
                while (currentDropIndex < this.container.getInventory().size()) {
                    if (!this.container.getSlot(currentDropIndex).getStack().isEmpty()) {
                        mc.playerController.windowClick(0, currentDropIndex, 0, ClickType.PICKUP, mc.player);
    ...
    protected void init() {
        if (this.minecraft.playerController.isInCreativeMode()) {
            ...
            this.addButton(new Button(width / 2 - 50, height / 2 - 110, 100, 20,
    ...
    private void startDroppingItems() {
    ...
    public void deleteall() {
        if (this.minecraft.player != null && this.minecraft.playerController != null) {
            new Thread(() -> {
                try {
                    for (int i = 0; i < 36; i++) {
                        if (this.minecraft.player.inventory.getStackInSlot(i).getItem() != Items.AIR) {
                            int finalI = i;
                            this.minecraft.execute(() -> {
                                int slot = finalI < 9 ? finalI + 36 : finalI;
                                this.minecraft.playerController.windowClick(0, slot, 45, ClickType.SWAP, this.minecraft.player);
                            });
                            Thread.sleep(50L);
                        }
                    }

                    if (this.minecraft.player.inventory.getStackInSlot(40).getItem() != Items.AIR) {
                        this.minecraft.execute(() -> {
                            this.minecraft.playerController.windowClick(0, 45, 45, ClickType.SWAP, this.minecraft.player);
                        });
                        Thread.sleep(50L);
                    }

                    for (int armorSlot = 0; armorSlot < 4; armorSlot++) {
                        if (this.minecraft.player.inventory.armorInventory.get(3 - armorSlot).getItem() != Items.AIR) {
                            int finalArmorSlot = armorSlot;
                            this.minecraft.execute(() -> {
                                this.minecraft.playerController.windowClick(0, 5 + finalArmorSlot, 45, ClickType.SWAP, this.minecraft.player);
                            });
                            Thread.sleep(50L);
                        }
                    }
