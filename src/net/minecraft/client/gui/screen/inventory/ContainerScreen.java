...
import xd.harm.modules.impl.misc.AhHelper;
import xd.harm.modules.impl.misc.InventoryPlus;
import xd.harm.modules.impl.player.ShulkerVisible;
import xd.harm.utils.render.color.ColorUtils;
import xd.harm.utils.render.VisualityGuiAnimationHelper;
...
public abstract class ContainerScreen<T extends Container> extends Screen implements IHasContainer<T>, red.jackf.chesttracker.mixins.AccessorHandledScreen, red.jackf.whereisit.mixin.AccessorHandledScreen {
    public static final ResourceLocation INVENTORY_BACKGROUND = new ResourceLocation("textures/gui/container/inventory.png");
    private static final ResourceLocation SHULKER_TOOLTIP = new ResourceLocation("harmony/images/shulker/shulker_box_tooltip.png");
    ...
    private float visualityAnimationDeltaTicks;
    private long visualityLastAnimationNanos;
    private float visualityCarriedAnimationProgress;
    private ItemStack visualityLastCarriedStack = ItemStack.EMPTY;
    ...
    public void render(MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        this.updateVisualityAnimationDelta();
        int i = this.guiLeft;
        int j = this.guiTop;
        ...
        for (int i1 = 0; i1 < this.container.inventorySlots.size(); ++i1) {
            ...
            if (this.isSlotSelected(slot, (double) mouseX, (double) mouseY) && slot.isEnabled()) {
                ...
                if (inventoryPlus.itemScroller.get() && inventoryPlus.isState()) {
        ...
        if (!itemstack.isEmpty()) {
            ...
            int itemX = mouseX - i - 8;
            int itemY = mouseY - j - k2;
            if (VisualityGuiAnimationHelper.isTinyItemAnimationsEnabled()) {
                this.updateVisualityCarriedAnimation(itemstack);
                this.drawAnimatedFloatingItem(itemstack, itemX, itemY, s);
            } else {
                this.resetVisualityCarriedAnimation();
                this.drawItemStack(itemstack, itemX, itemY, s);
            }
        } else {
            this.resetVisualityCarriedAnimation();
        }
        ...
        RenderSystem.enableDepthTest();
        xd.harm.chesttracker.HarmonyChestTrackerBootstrap.fireScreenLateRender(matrixStack, this, mouseX, mouseY, partialTicks);
    }
    ...
    protected void renderHoveredTooltip(MatrixStack matrixStack, int mouseX, int mouseY) {
        if (this.minecraft.player.inventory.getItemStack().isEmpty() && this.hoveredSlot != null && this.hoveredSlot.getHasStack()) {
            ...
            ModuleManager moduleManager = Harmony.getInstance().getModuleManager();
            ShulkerVisible shulkerVisible = moduleManager.getShulkerVisible();

            if (Block.getBlockFromItem(itemStack.getItem()) instanceof ShulkerBoxBlock
                    && shulkerVisible.isState()
                    && shulkerVisible.showInInventory.get()) {
                renderShulkerTooltip(matrixStack, mouseX, mouseY, itemStack);
            } else {
    ...
    private void renderShulkerTooltip(MatrixStack matrixStack, int mouseX, int mouseY, ItemStack shulkerStack) {
        ShulkerBoxBlock shulkerBlock = (ShulkerBoxBlock) Block.getBlockFromItem(shulkerStack.getItem());
        List<ItemStack> items = getItemsInShulker(shulkerStack);

        List<ITextComponent> tooltip = shulkerStack.getTooltip(this.minecraft.player,
                this.minecraft.gameSettings.advancedItemTooltips ? ITooltipFlag.TooltipFlags.ADVANCED : ITooltipFlag.TooltipFlags.NORMAL);
        ...
        int sx = mouseX + 12;
        int sy = mouseY + tooltipHeight + 8;

        if (sx + width > this.minecraft.getMainWindow().getScaledWidth()) {
            sx = mouseX - width - 4;
        }
        if (sy + height > this.minecraft.getMainWindow().getScaledHeight()) {
            sy = mouseY - height - tooltipHeight - 4;
        }
        ...
        if (color != null) {
            ...
            RenderSystem.color4f(0.54f, 0.37f, 0.54f, 1.0f);
        }

        RenderSystem.enableBlend();
        RenderSystem.defaultBlendFunc();

        this.minecraft.getTextureManager().bindTexture(SHULKER_TOOLTIP);
        this.blit(matrixStack, sx, sy, 0, 0, width, height);
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            if (!stack.isEmpty()) {
                ...
                this.minecraft.getItemRenderer().renderItemIntoGUI(stack, sx + offX, sy + offY);
                this.minecraft.getItemRenderer().renderItemOverlays(this.minecraft.fontRenderer, stack, sx + offX, sy + offY);
                RenderSystem.popMatrix();
            }
        ...
        if (tooltip.size() > 0) {
            this.renderTooltip(matrixStack, shulkerStack, mouseX, mouseY);
        }
    }
    ...
    private void updateVisualityAnimationDelta() {
        long now = System.nanoTime();
        if (this.visualityLastAnimationNanos == 0L) {
            this.visualityAnimationDeltaTicks = 0.0F;
        } else {
            this.visualityAnimationDeltaTicks = MathHelper.clamp((now - this.visualityLastAnimationNanos) / 50000000.0F, 0.0F, 4.0F);
        }
        this.visualityLastAnimationNanos = now;
    }

    private void updateVisualityCarriedAnimation(ItemStack stack) {
        if (!ItemStack.areItemStacksEqual(this.visualityLastCarriedStack, stack)) {
            this.visualityLastCarriedStack = stack.isEmpty() ? ItemStack.EMPTY : stack.copy();
            this.visualityCarriedAnimationProgress = 0.0F;
        }
        this.visualityCarriedAnimationProgress = VisualityGuiAnimationHelper.advanceProgress(this.visualityCarriedAnimationProgress, this.visualityAnimationDeltaTicks);
    }

    private void resetVisualityCarriedAnimation() {
        this.visualityCarriedAnimationProgress = 0.0F;
        this.visualityLastCarriedStack = ItemStack.EMPTY;
    }

    private void drawAnimatedFloatingItem(ItemStack stack, int x, int y, @Nullable String altText) {
        float scale = VisualityGuiAnimationHelper.getScale(this.visualityCarriedAnimationProgress);
        if (scale != 1.0F) {
        ...
        this.drawItemStack(stack, x, y, altText);
        if (scale != 1.0F) {
            RenderSystem.popMatrix();
        }
    }
    ...
    private void moveItems(MatrixStack matrixStack, Slot slot) {
        ...
        boolean tinyItemAnimations = VisualityGuiAnimationHelper.isTinyItemAnimationsEnabled();
        slot.syncVisualityAnimation(itemstack, tinyItemAnimations);
        boolean flag = false;
        ...
        if (inventoryPlus.isState() && inventoryPlus.itemHelper.get() && !itemstack.isEmpty()) {
            if (inventoryPlus.shouldHighlight(itemstack)) {
                ...
                fill(matrixStack, i, j, i + 16, j + 16, animatedColor);
        ...
        AhHelper ahHelper = moduleManager.getAhHelper();
        if (ahHelper.isState()) {
            if (ahHelper.getX() != 0) {
                int x = (int) ahHelper.getX();
                int y = (int) ahHelper.getY();
                fill(matrixStack, x, y, x + 16, y + 16, ColorUtils.rgba(64, 255, 64, 3));
                if (ahHelper.getX2() != 0) {
                    int x2 = (int) ahHelper.getX2();
                    int y2 = (int) ahHelper.getY2();
                    fill(matrixStack, x2, y2, x2 + 16, y2 + 16, ColorUtils.rgba(255, 255, 64, 3));
                }
                if (ahHelper.getX3() != 0) {
                    int x3 = (int) ahHelper.getX3();
                    int y3 = (int) ahHelper.getY3();
                    fill(matrixStack, x3, y3, x3 + 16, y3 + 16, ColorUtils.rgba(255, 64, 64, 3));
                }
            }
        }
        ...
        if (!flag1) {
            ...
            if (tinyItemAnimations && !slot.getStack().isEmpty()) {
                itemScale = VisualityGuiAnimationHelper.getSlotScale(slot, this.visualityAnimationDeltaTicks);
                if (itemScale != 1.0F) {
                    float centerX = i + 8.0F;
                    float centerY = j + 8.0F;
                    RenderSystem.pushMatrix();
                    RenderSystem.translatef(centerX, centerY, 0.0F);
