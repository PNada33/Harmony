...
import xd.harm.HarmonyStartupException;
...
public class Main {
    ...
    public static void main(String[] p_main_0_) {
        ...
        OptionSpec<String> optionspec10 = optionparser.accepts("username").withRequiredArg().defaultsTo("HarmonyBoost" + Util.milliTime() % 1000L);
        ...
        Thread thread = new Thread("Client Shutdown Thread") {
            public void run() {
                ...
                DragManager.save();
                xd.harm.modules.impl.misc.BotAttack.shutdownAll();
                Harmony.getInstance().getConfigStorage().saveConfiguration("backup");
        ...
        } catch (HarmonyStartupException startupException) {
            LOGGER.error(startupException.getMessage());
            return;
        } catch (UndeclaredException undeclaredexception) {
            LOGGER.warn("Failed to create window: ", (Throwable) undeclaredexception);
            return;
        } catch (Throwable throwable1) {
            if (containsHarmonyStartupException(throwable1)) {
                LOGGER.error("Cloud bootstrap unavailable");
                return;
            }
    ...
    private static boolean containsHarmonyStartupException(Throwable throwable) {
        Throwable current = throwable;
        while (current != null) {
            if (current instanceof HarmonyStartupException) {
                return true;
            }
