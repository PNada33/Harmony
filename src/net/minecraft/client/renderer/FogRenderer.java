...
public class FogRenderer
{
    ...
    public static void updateFogColor(ActiveRenderInfo activeRenderInfoIn, float partialTicks, ClientWorld worldIn, int renderDistanceChunks, float bossColorModifier)
    {
        ...
        else
        {
            ...
            WorldTweaks worldTweaks = Harmony.getInstance().getModuleManager().getWorldTweaks();
            red = (float)vector3d4.getX();
            green = (float)vector3d4.getY();
            blue = (float)vector3d4.getZ();

            if (worldTweaks.isState() && worldTweaks.options.getValueByName("Кастомный туман").get()) {
                final float[] colors = ColorUtils.rgba(worldTweaks.mode.is("Клиент") ? ColorUtils.getColor(10) : worldTweaks.colorFog.get());
                red = colors[0];
                green = colors[1];
                blue = colors[2];
            }
            ...
            if (renderDistanceChunks >= 4)
            {
                ...
                float f17 = activeRenderInfoIn.getViewVector().getX() * f13;
