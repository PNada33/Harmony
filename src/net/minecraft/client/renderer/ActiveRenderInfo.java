...
import xd.harm.modules.impl.render.NoRender;
...
public class ActiveRenderInfo {
    ...
    private float smallPlayerCameraOffset;

    private boolean firsttime;

    private CameraEvent event = new CameraEvent(0, 0, 0);
    ...
    public void update(IBlockReader worldIn, Entity renderViewEntity, boolean thirdPersonIn, boolean thirdPersonReverseIn, float partialTicks)
    {
        ...
        float yaw = renderViewEntity.getYaw(partialTicks);
        float pitch = renderViewEntity.getPitch(partialTicks);
        ...
        boolean smoothThirdPerson = visuality != null && visuality.isSmoothCameraActive() && thirdPersonIn;
        if (visuality != null) {
            visuality.updateSmoothCameraState(renderViewEntity, thirdPersonIn, event.yaw, event.pitch);
        }
        float finalYaw = smoothThirdPerson
                ? visuality.getInterpolatedCameraYaw(partialTicks, event.yaw)
                : event.yaw;
        float finalPitch = smoothThirdPerson
                ? visuality.getInterpolatedCameraPitch(partialTicks, event.pitch)
                : event.pitch;
        this.setDirection(finalYaw, finalPitch);
        double rawX = MathHelper.lerp((double) partialTicks, renderViewEntity.prevPosX, renderViewEntity.getPosX());
        double rawY = MathHelper.lerp((double) partialTicks, renderViewEntity.prevPosY, renderViewEntity.getPosY());
        double rawZ = MathHelper.lerp((double) partialTicks, renderViewEntity.prevPosZ, renderViewEntity.getPosZ());
        float targetCameraOffset = WorldTweaks.isSmallPlayerEntity(renderViewEntity)
                ? WorldTweaks.getSmallPlayerCameraYOffset(renderViewEntity)
                : 0.0F;
        this.smallPlayerCameraOffset += (targetCameraOffset - this.smallPlayerCameraOffset) * 0.18F;
        if (Math.abs(this.smallPlayerCameraOffset - targetCameraOffset) < 1.0E-4F) {
            this.smallPlayerCameraOffset = targetCameraOffset;
        }
        double baseY = rawY + (double) MathHelper.lerp(partialTicks, this.previousHeight, this.height) + this.smallPlayerCameraOffset;
        this.setPosition(rawX, baseY, rawZ);
        ...
        if (thirdPersonIn)
        {
            ...
            double cameraDistance = smoothThirdPerson
                    ? visuality.getInterpolatedDistance(partialTicks)
                    : 4.0D;
            double heightOffset = smoothThirdPerson
                    ? visuality.getInterpolatedHeightOffset(partialTicks)
                    : 0.0D;
            this.movePosition(-this.calcCameraDistance(cameraDistance), heightOffset, 0.0D);
        }
    ...
    private double interpolatedDistance;
    ...
    private double calcCameraDistance(double startingDistance) {
        ...
        NoRender noRender = Harmony.getInstance().getModuleManager().getNoRender();
        return noRender.isState()  ? 4 : startingDistance;
    }
    ...
    public void clear() {
        ...
        this.valid = false;
        this.smallPlayerCameraOffset = 0.0F;
    }
}
