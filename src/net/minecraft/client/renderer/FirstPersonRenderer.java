...
public class FirstPersonRenderer {
    ...
    private void renderItemInFirstPerson(AbstractClientPlayerEntity player, float partialTicks, float pitch, Hand handIn, float swingProgress, ItemStack stack, float equippedProgress, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int combinedLightIn) {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand(handIn)) {
            ...
            if (stack.isEmpty()) {
                ...
                ModuleManager functionRegistry = Harmony.getInstance().getModuleManager();
                ...
                if (player.isHandActive() && player.getItemInUseCount() > 0 && player.getActiveHand() == handIn) {
                    ...
                    switch (stack.getUseAction()) {
                        ...
                        case BLOCK:
                            {
                                SwingAnimation sa = Harmony.getInstance().getModuleManager().getSwingAnimation();
                                if (sa.blockAnimation.get() && sa.shouldAnimate() && !sa.isVanillaType15()) {
                                    int l = flag3 ? 1 : -1;
                                    float ep = 1.0F - (this.prevEquippedProgressMainHand + (this.equippedProgressMainHand - this.prevEquippedProgressMainHand) * partialTicks);
                                    sa.generateSwing(matrixStackIn, l, swingProgress, ep);
                                } else {
                                    this.transformSideFirstPerson(matrixStackIn, handside, equippedProgress);
                                }
                            }
                    ...
                    int l = flag3 ? 1 : -1;
                    float equipProgress = 1.0F - (this.prevEquippedProgressMainHand + (this.equippedProgressMainHand - this.prevEquippedProgressMainHand) * partialTicks);

                    // Тип 15 (Mod 6) -- чистая ваниль, как в AndroQuantum:
                    // не вызываем generateSwing, идём по ванильному пути.
                    if (flag && swingAnimation.shouldAnimate() && !swingAnimation.isVanillaType15()) {
                        swingAnimation.generateSwing(matrixStackIn, l, swingProgress, equipProgress);
                    } else {
                        matrixStackIn.translate((double) ((float) l * f6), (double) f7, (double) f10);
                        this.transformSideFirstPerson(matrixStackIn, handside, equippedProgress);
                        this.transformFirstPerson(matrixStackIn, handside, swingProgress);
                    }
                    swingAnimation.processSpinRotation(matrixStackIn, handside);
                }

                SwingAnimation sa = Harmony.getInstance().getModuleManager().getSwingAnimation();
                if (!(sa.removeShield.get() && sa.blockAnimation.get() && stack.getItem() instanceof ShieldItem)) {
    ...
    public void tick() {
        ...
        if (clientplayerentity.isRowingBoat()) {
            ...
            SwingAnimation swingAnim = Harmony.getInstance().getModuleManager().getSwingAnimation();
            if (swingAnim != null && swingAnim.isState() && swingAnim.oldCooldownAnimation.get()
                    && this.itemStackMainHand == itemstack) {
