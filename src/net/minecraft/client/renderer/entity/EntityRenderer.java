...
import xd.harm.modules.impl.render.NameTags;
...
public abstract class EntityRenderer<T extends Entity> implements net.optifine.entity.model.IEntityRenderer {
    ...
    protected void renderName(T entityIn, ITextComponent displayNameIn, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
        ...
        NameTags nameTags = moduleManager.getNameTags();

        if (nameTags.isState() && entityIn instanceof PlayerEntity p) {
            if (nameTags.isValid(p)) {
                return;
            }
        }
        ...
        if (flag) {
            ...
            if (WorldTweaks.isSmallPlayerEntity(entityIn)) {
                f = entityIn.getHeight() * WorldTweaks.getSmallPlayerRenderScale(entityIn) + 0.5F;
            }
            ...
            if (!Boolean.getBoolean("bot.mode")) {
                HarmonyVoicechatBootstrap.fireRenderNameplate(entityIn, displayNameIn, matrixStackIn, bufferIn, packedLightIn);
            }
        }
    }
