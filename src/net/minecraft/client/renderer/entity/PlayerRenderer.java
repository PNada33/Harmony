...
import xd.harm.utils.waveycapes.renderlayers.CustomCapeRenderLayer;
...
public class PlayerRenderer extends LivingRenderer<AbstractClientPlayerEntity, PlayerModel<AbstractClientPlayerEntity>>
{
    ...
    public void render(AbstractClientPlayerEntity entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn)
    {
        ...
        // Suppress vanilla player model when a Figura avatar is active
        try {
            Class<?> wearCl = Class.forName("xd.harm.utils.figura.FiguraWear");
            java.lang.reflect.Method getCurrent = wearCl.getMethod("getCurrent");
            Object cur = getCurrent.invoke(null);
            if (cur != null) {
                // Skip base player render; still allow overlays like voice icon
                if (!Boolean.getBoolean("bot.mode")) {
                    HarmonyVoicechatBootstrap.fireRenderPlayerVoiceIcon(entityIn, matrixStackIn, bufferIn, packedLightIn);
                }
                return;
            }
    ...
    {
        double y = entityIn.isCrouching() ? -0.125D : 0.0D;
        if (WorldTweaks.isSmallPlayerEntity(entityIn)) {
            y += WorldTweaks.getSmallPlayerYOffset(entityIn);
        }
    ...
    protected void applyRotations(AbstractClientPlayerEntity entityLiving, MatrixStack matrixStackIn, float ageInTicks, float rotationYaw, float partialTicks)
    {
        ...
        if (entityLiving.isElytraFlying())
        {
            ...
            if (d0 > 0.0D && d1 > 0.0D)
            {
                ...
                float glideTurn = (float)Math.atan2(d3, MathHelper.clamp(d2, -1.0D, 1.0D));
                glideTurn = MathHelper.clamp(glideTurn, -0.85F, 0.85F);
                matrixStackIn.rotate(Vector3f.YP.rotation(glideTurn * f2));
            }
        }
        else if (f > 0.0F)
        {
