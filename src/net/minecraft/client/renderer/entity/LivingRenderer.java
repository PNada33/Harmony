...
import xd.harm.utils.figura.CosmeticPat;
...
public abstract class LivingRenderer<T extends LivingEntity, M extends EntityModel<T>> extends EntityRenderer<T> implements IEntityRenderer<T, M> {
    ...
    public void render(T entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
        if (!Reflector.RenderLivingEvent_Pre_Constructor.exists() || !Reflector.postForgeBusEvent(Reflector.RenderLivingEvent_Pre_Constructor, entityIn, this, partialTicks, matrixStackIn, bufferIn, packedLightIn)) {
            ...
            boolean smallPlayer = WorldTweaks.isSmallPlayerEntity(entityIn);
            if (smallPlayer) {
            ...
            float patScale = CosmeticPat.get().getEntityScale(entityIn);
            if (patScale != 1.0F) {
                matrixStackIn.scale(1.0F, patScale, 1.0F);
            }
            matrixStackIn.translate(0.0D, (double) -1.501F, 0.0D);
            if (smallPlayer) {
                float smallScale = WorldTweaks.getSmallPlayerRenderScale(entityIn);
                matrixStackIn.scale(smallScale, smallScale, smallScale);
            }
            float f9 = 0.0F;
            float f5 = 0.0F;
            ...
            boolean flag3 = minecraft.isEntityGlowing(entityIn) && !Harmony.getInstance().getModuleManager().getNoRender().element.getValueByName("Эффект свечения").get();
            ...
            if (rendertype != null) {
                ...
                this.entityModel.render(matrixStackIn, ivertexbuilder, packedLightIn, i, 1.0F, 1.0F, 1.0F, flag2 ? (blurblat.isState() ? blurblat.alpha.get().floatValue() : 0.15F) : 1.0F);
            }
