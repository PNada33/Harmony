...
public class OverlayRenderer {
    ...
    private static Pair<BlockState, BlockPos> getOverlayBlock(PlayerEntity p_getOverlayBlock_0_) {
        ...
        for (int i = 0; i < 8; ++i) {
            double d0 = p_getOverlayBlock_0_.getPosX()
                    + (double) (((float) ((i >> 0) % 2) - 0.5F) * p_getOverlayBlock_0_.getWidth() * 0.8F);
            double d1 = p_getOverlayBlock_0_.getPosYEye() + (double) (((float) ((i >> 1) % 2) - 0.5F) * 0.1F);
            double d2 = p_getOverlayBlock_0_.getPosZ()
                    + (double) (((float) ((i >> 2) % 2) - 0.5F) * p_getOverlayBlock_0_.getWidth() * 0.8F);
            ...
            if (blockstate.getRenderType() != BlockRenderType.INVISIBLE
                    && blockstate.causesSuffocation(p_getOverlayBlock_0_.world, blockpos$mutable)) {
    ...
    private static void renderUnderwater(Minecraft minecraftIn, MatrixStack matrixStackIn) {
        EventCancelOverlay eventCancelOverlay = new EventCancelOverlay(EventCancelOverlay.Overlays.UNDER_WATER);
        ...
        if (!Config.isShaders() || Shaders.isUnderwaterOverlay()) {
            ...
            if (SmartAnimations.isActive()) {
                SmartAnimations.textureRendered(
                        minecraftIn.getTextureManager().getTexture(TEXTURE_UNDERWATER).getGlTextureId());
            }
