...
public class SimpleBakedModel implements IBakedModel
{
    ...
    {
        this.hasTranslucency = this.texture != null && this.texture.hasTranslucency();

        if (!this.hasTranslucency)
        {
            for (BakedQuad bakedquad : this.generalQuads)
            {
                if (bakedquad.getSprite() != null && bakedquad.getSprite().hasTranslucency())
                {
            ...
            for (List<BakedQuad> list : this.faceQuads.values())
            {
                for (BakedQuad bakedquad1 : list)
                {
                    if (bakedquad1.getSprite() != null && bakedquad1.getSprite().hasTranslucency())
                    {
                        this.hasTranslucency = true;
                        return;
                    }
