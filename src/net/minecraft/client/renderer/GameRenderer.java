...
import xd.harm.modules.impl.combat.NoEntityTrace;
import xd.harm.modules.impl.misc.Visuality;
import xd.harm.modules.impl.render.WorldTweaks;
import xd.harm.ui.mainmenu.MainScreen;
import xd.harm.utils.client.HarmonyGuiScale;
import xd.harm.baritone.api.BaritoneAPI;
import xd.harm.baritone.api.IBaritone;
import xd.harm.baritone.api.event.events.RenderEvent;
...
public class GameRenderer implements IResourceManagerReloadListener, AutoCloseable {
    ...
    private void loadShader(ResourceLocation resourceLocationIn) {
        if (GLX.isUsingFBOs()) {
            ...
            try {
                ...
                this.shaderGroup.createBindFramebuffers(this.mc.getMainWindow().getFramebufferWidth(),
    ...
    public void getMouseOver(float partialTicks) {
        ...
        if (entity != null && this.mc.world != null) {
            ...
            boolean useSmallPlayerPickOffset = false;

            if (entity == this.mc.player
                    && this.mc.gameSettings.getPointOfView().func_243192_a()
                    && WorldTweaks.isSmallPlayerEntity(entity)) {
                float pickYOffset = WorldTweaks.getSmallPlayerCameraYOffset(entity);
                if (Math.abs(pickYOffset) > 1.0E-4F) {
                    vector3d = vector3d.add(0.0D, pickYOffset, 0.0D);
                    useSmallPlayerPickOffset = true;
                }
            }

            if (useSmallPlayerPickOffset) {
                Vector3d lookVec = entity.getLook(1.0F);
                Vector3d blockReachVec = vector3d.add(lookVec.x * d0, lookVec.y * d0, lookVec.z * d0);
                this.mc.objectMouseOver = this.mc.world.rayTraceBlocks(new RayTraceContext(vector3d, blockReachVec,
            ...
            NoEntityTrace noEntityTrace = moduleManager.getNoEntityTrace();

            if (!noEntityTrace.isState()) {
                ...
                EntityRayTraceResult entityraytraceresult = ProjectileHelper.rayTraceEntities(entity, vector3d, vector3d2,
                        axisalignedbb, (p_lambda$getMouseOver$0_0_) -> {
                            return !p_lambda$getMouseOver$0_0_.isSpectator()
                                    && p_lambda$getMouseOver$0_0_.canBeCollidedWith();
                        }, d1);
                ...
                if (entityraytraceresult != null) {
                    ...
                    if (flag && d2 > 9.0D) {
                        this.mc.objectMouseOver = BlockRayTraceResult.createMiss(vector3d3,
                                Direction.getFacingFromVector(vector3d1.x, vector3d1.y, vector3d1.z),
                                new BlockPos(vector3d3));
    ...
    private void updateFovModifierHand() {
        ...
        if (this.mc.getRenderViewEntity() instanceof AbstractClientPlayerEntity) {
            AbstractClientPlayerEntity abstractclientplayerentity = (AbstractClientPlayerEntity) this.mc
                    .getRenderViewEntity();
    ...
    private final Animation zoomAnim = new Animation().setToValue(1);
    ...
    public double getFOVModifier(ActiveRenderInfo activeRenderInfoIn, float partialTicks, boolean useFOVSetting) {
        if (this.debugView) {
            ...
            ModuleManager ModuleManager = Harmony.getInstance().getModuleManager();
            Visuality visuality = ModuleManager.getVisuality();
            boolean isBetter = visuality != null && visuality.isState() && visuality.smoothCamera.get();

            if (isBetter) {
                if (lastFlag != flag) {
                    zoomAnim.animate(flag ? 4 : 1, 0.5, Easings.QUINT_OUT);
                }

                lastFlag = flag;
                zoomAnim.update();
                d0 /= zoomAnim.getValue();
            }
            ...
            if (activeRenderInfoIn.getRenderViewEntity() instanceof LivingEntity
                    && ((LivingEntity) activeRenderInfoIn.getRenderViewEntity()).getShouldBeDead()) {
                float f = Math.min(
                        (float) ((LivingEntity) activeRenderInfoIn.getRenderViewEntity()).deathTime + partialTicks,
                        20.0F);
            ...
            return Reflector.ForgeHooksClient_getFOVModifier.exists()
                    ? Reflector.callDouble(Reflector.ForgeHooksClient_getFOVModifier, this, activeRenderInfoIn,
                    partialTicks, d0)
                    : d0;
        }
    }
    ...
    private void applyBobbing(MatrixStack matrixStackIn, float partialTicks) {
        if (this.mc.getRenderViewEntity() instanceof PlayerEntity) {
            boolean firstPersonView = this.mc.gameSettings.getPointOfView().func_243192_a();
            boolean visualityFallBobbing = !firstPersonView
                    && Visuality.isFallingAnimationEnabled()
                    && mc.player.fallDistance > 2.0F;
            if (mc.player.isElytraFlying() || visualityFallBobbing) {
            ...
            } else if (mc.player.isOnGround() || mc.gameSettings.keyBindJump.isPressed()) {
    ...
    public void renderHand(MatrixStack p_renderHand_1_, ActiveRenderInfo p_renderHand_2_, float p_renderHand_3_,
                           boolean p_renderHand_4_, boolean p_renderHand_5_, boolean p_renderHand_6_) {

        if (!this.debugView) {
            xd.harm.modules.impl.render.ShaderHand shaderHand = xd.harm.modules.impl.render.ShaderHand.getInstance();
            boolean useShaderHand = shaderHand != null && shaderHand.isState()
                    && this.mc.gameSettings.getPointOfView().func_243192_a();

            if (useShaderHand) {
                net.minecraft.client.shader.Framebuffer fbo = shaderHand.getHandsBuffer();
            ...
            if (p_renderHand_4_) {
                ...
                flag = this.mc.getRenderViewEntity() instanceof LivingEntity
                        && ((LivingEntity) this.mc.getRenderViewEntity()).isSleeping();

                if (this.mc.gameSettings.getPointOfView().func_243192_a() && !flag && !this.mc.gameSettings.hideGUI
                        && this.mc.playerController.getCurrentGameType() != GameType.SPECTATOR) {
                    this.lightmapTexture.enableLightmap();

                    if (Config.isShaders()) {
                        ShadersRender.renderItemFP(this.itemRenderer, p_renderHand_3_, p_renderHand_1_,
                                this.renderTypeBuffers.getBufferSource(), this.mc.player,
                                this.mc.getRenderManager().getPackedLight(this.mc.player, p_renderHand_3_),
                                p_renderHand_6_);
                    } else {
                        this.itemRenderer.renderItemInFirstPerson(p_renderHand_3_, p_renderHand_1_,
                                this.renderTypeBuffers.getBufferSource(), this.mc.player,
                                this.mc.getRenderManager().getPackedLight(this.mc.player, p_renderHand_3_));
                    }
        ...
        matrixstack.getLast().getMatrix()
                .mul(Matrix4f
                        .perspective(this.getFOVModifier(activeRenderInfoIn, partialTicks, useFovSetting),
                                (float)this.mc.getMainWindow().getFramebufferWidth() / (float)this.mc.getMainWindow().getFramebufferHeight(), 0.05f, this.clipDistance));
    ...
    public void updateCameraAndRender(float partialTicks, long nanoTime, boolean renderWorldIn) {
        ...
        if (!this.mc.isGameFocused() && this.mc.gameSettings.pauseOnLostFocus
                && (!this.mc.gameSettings.touchscreen || !this.mc.mouseHelper.isRightDown())) {
        ...
        if (!this.mc.skipRenderWorld) {
            int i = (int) (this.mc.mouseHelper.getMouseX() * (double) this.mc.getMainWindow().getScaledWidth()
                    / (double) this.mc.getMainWindow().getWidth());
            int j = (int) (this.mc.mouseHelper.getMouseY() * (double) this.mc.getMainWindow().getScaledHeight()
            ...
            if (renderWorldIn && this.mc.world != null && !Config.isReloadingResources()) {
                ...
                RenderSystem.viewport(0, 0, this.mc.getMainWindow().getFramebufferWidth(),
                        this.mc.getMainWindow().getFramebufferHeight());
            }
            ...
            RenderSystem.matrixMode(5889);
            RenderSystem.loadIdentity();
            RenderSystem.ortho(0.0D, (double) mainwindow.getFramebufferWidth() / mainwindow.getGuiScaleFactor(),
                    (double) mainwindow.getFramebufferHeight() / mainwindow.getGuiScaleFactor(), 0.0D, 1000.0D,
                    3000.0D);
            RenderSystem.matrixMode(5888);
            RenderSystem.loadIdentity();
            ...
            if (renderWorldIn && this.mc.world != null) {
                ...
                if (this.mc.player != null) {
                    float f = MathHelper.lerp(partialTicks, this.mc.player.prevTimeInPortal,
                            this.mc.player.timeInPortal);

                    if (f > 0.0F && this.mc.player.isPotionActive(Effects.NAUSEA)
                            && this.mc.gameSettings.screenEffectScale < 1.0F) {
                ...
                if (!this.mc.gameSettings.hideGUI || this.mc.currentScreen != null) {
                    ...
                    this.renderItemActivation(this.mc.getMainWindow().getScaledWidth(),
                            this.mc.getMainWindow().getScaledHeight(), partialTicks);
            ...
            if (this.mc.loadingGui != null) {
                ...
                boolean harmonyScreen = HarmonyGuiScale.isHarmonyScreen(this.mc.currentScreen);
                int screenMouseX = i;
                int screenMouseY = j;

                if (harmonyScreen) {
                    this.setupOverlayRendering(HarmonyGuiScale.FIXED_GUI_SCALE);
                    screenMouseX = HarmonyGuiScale.getFixedMouseX(this.mc);
                    screenMouseY = HarmonyGuiScale.getFixedMouseY(this.mc);
                }

                try {
                    if (Reflector.ForgeHooksClient_drawScreen.exists()) {
                        Reflector.callVoid(Reflector.ForgeHooksClient_drawScreen, this.mc.currentScreen, matrixstack,
                                screenMouseX, screenMouseY, this.mc.getTickLength());
                    } else {
                        this.mc.currentScreen.render(matrixstack, screenMouseX, screenMouseY, this.mc.getTickLength());
                    }
                } catch (Throwable throwable1) {
                    final int crashMouseX = screenMouseX;
                    final int crashMouseY = screenMouseY;
                    ...
                    crashreportcategory1.addDetail("Mouse location", () -> {
                        return String.format(Locale.ROOT, "Scaled: (%d, %d). Absolute: (%f, %f)", crashMouseX, crashMouseY,
                                this.mc.mouseHelper.getMouseX(), this.mc.mouseHelper.getMouseY());
                    });
                    crashreportcategory1.addDetail("Screen size", () -> {
                        return String.format(Locale.ROOT, "Scaled: (%d, %d). Absolute: (%d, %d). Scale factor of %f",
                                this.mc.getMainWindow().getScaledWidth(), this.mc.getMainWindow().getScaledHeight(),
            ...
            this.setupOverlayRendering(HarmonyGuiScale.FIXED_GUI_SCALE);
            EventDisplay screenDisplay = new EventDisplay(matrixstack, partialTicks);
            screenDisplay.setType(EventDisplay.Type.POST_SCREEN);
            Harmony.getInstance().getEventBus().post(screenDisplay);
            this.setupOverlayRendering();
    ...
    private void createWorldIcon() {
        if (this.mc.worldRenderer.getRenderedChunks() > 10 && this.mc.worldRenderer.hasNoChunkUpdates()
                && !this.mc.getIntegratedServer().isWorldIconSet()) {
            NativeImage nativeimage = ScreenShotHelper.createScreenshot(this.mc.getMainWindow().getFramebufferWidth(),
                    this.mc.getMainWindow().getFramebufferHeight(), this.mc.getFramebuffer());
            ...
            if (flag && !((PlayerEntity) entity).abilities.allowEdit) {
                ...
                if (raytraceresult != null && raytraceresult.getType() == RayTraceResult.Type.BLOCK) {
                    ...
                    if (this.mc.playerController.getCurrentGameType() == GameType.SPECTATOR) {
                        ...
                        flag = !itemstack.isEmpty() && (itemstack.canDestroy(this.mc.world.getTags(), cachedblockinfo)
                                || itemstack.canPlaceOn(this.mc.world.getTags(), cachedblockinfo));
                    }
                }
            }
    ...
    public void renderWorld(float partialTicks, long finishTimeNano, MatrixStack matrixStackIn) {
        ...
        float f = MathHelper.lerp(partialTicks, this.mc.player.prevTimeInPortal, this.mc.player.timeInPortal)
                * this.mc.gameSettings.screenEffectScale * this.mc.gameSettings.screenEffectScale;
        ...
        activerenderinfo.update(this.mc.world,
                (Entity) (this.mc.getRenderViewEntity() == null ? this.mc.player : this.mc.getRenderViewEntity()),
                !this.mc.gameSettings.getPointOfView().func_243192_a(),
                this.mc.gameSettings.getPointOfView().func_243193_b(), partialTicks);
        ...
        if (Reflector.ForgeHooksClient_dispatchRenderLast.exists()) {
            ...
            Reflector.callVoid(Reflector.ForgeHooksClient_dispatchRenderLast, this.mc.worldRenderer, matrixStackIn,
                    partialTicks, matrix4f, finishTimeNano);
        }
    ...
    private void frameInit() {
        ...
        if (world != null) {
            if (Config.getNewRelease() != null) {
                ...
                stringtextcomponent.setStyle(Style.EMPTY
                        .setClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, "https://optifine.net/downloads")));
    ...
    public boolean setFxaaShader(int p_setFxaaShader_1_) {
        if (!GLX.isUsingFBOs()) {
            return false;
        } else if (this.shaderGroup != null && this.shaderGroup != this.fxaaShaders[2]
                && this.shaderGroup != this.fxaaShaders[4]) {
            return true;
    ...
    private void renderItemActivation(int widthsp, int heightScaled, float partialTicks) {
        if (this.itemActivationItem != null && this.itemActivationTicks > 0) {
            ...
            MatrixStack matrixstack = new MatrixStack();
            matrixstack.push();
            matrixstack.translate((double) ((float) (widthsp / 2) + f5 * MathHelper.abs(MathHelper.sin(f4 * 2.0F))),
                    (double) ((float) (heightScaled / 2) + f6 * MathHelper.abs(MathHelper.sin(f4 * 2.0F))), -50.0D);
            ...
            this.mc.getItemRenderer().renderItem(this.itemActivationItem, ItemCameraTransforms.TransformType.FIXED,
                    15728880, OverlayTexture.NO_OVERLAY, matrixstack, irendertypebuffer$impl);
            matrixstack.pop();
