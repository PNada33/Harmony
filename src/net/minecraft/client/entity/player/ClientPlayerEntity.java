...
import xd.harm.events.movement.ActionEvent;
...
import xd.harm.baritone.api.event.events.ChatEvent;
import xd.harm.baritone.api.event.events.PlayerUpdateEvent;
...
import xd.harm.modules.impl.player.AutoSkin;
import xd.harm.utils.math.StopWatch;
import net.minecraft.util.ResourceLocation;
import xd.harm.events.world.EventLivingTick;
import xd.harm.events.world.EventUpdate;
import xd.harm.events.world.InventoryCloseEvent;
...
public class ClientPlayerEntity extends AbstractClientPlayerEntity {
    ...
    public void tick() {
        if (this.world.isBlockLoaded(new BlockPos(this.getPosX(), 0.0D, this.getPosZ()))) {
            if (!Harmony.getInstance().playerOnServer) {
                Harmony.getInstance().playerOnServer = true;
            }

            Harmony.getInstance().getEventBus().post(eventUpdate);
            super.tick();
    ...
    private final EventMotion eventMotion = new EventMotion(0, 0, 0, 0, 0, false, new Vector3d(0, 0, 0), null);
    ...
    private void onUpdateWalkingPlayer() {
        ...
        ModuleManager mm = Harmony.getInstance().getModuleManager();
        if (mm.getMovementFix().isBackwardFix() && mm.getAutoSprint().isOmnidirectional() && this.movementInput.moveForward <= 0) {
            flag = false;
        }

        ActionEvent ea = new ActionEvent(flag);
        Harmony.getInstance().getEventBus().post(ea);
        ...
        if (this.isCurrentViewEntity()) {
            ...
            Harmony.getInstance().getEventBus().post(eventMotion);
        ...
        Harmony.getInstance().getEventBus().post(new EventPostMotion());
    ...
    public void sendChatMessage(String message) {
        ...
        CommandDispatcher commandDispatcher = Harmony.getInstance().getCommandDispatcher();

        if (commandDispatcher != null && commandDispatcher.dispatch(message) == DispatchResult.DISPATCHED) {
            return;
        }

        if (BotSessionManager.getInstance().routeChatToControlled(message)) {
            return;
        }
    ...
    private boolean shouldBlockPushPlayer(BlockPos pos) {
        ...
        if (antiPush.isState() && antiPush.modes.getValueByName("Блоки").get()) {
            return false;
        }
    ...
    public void setSprinting(boolean sprinting) {
        Harmony.getInstance().getEventBus().post(sprintEvent);
    ...
    public void livingTick() {
        ...
        if (this.isHandActive() && !this.isPassenger()) {
            Harmony.getInstance().getEventBus().post(noSlowEvent);
            final EventNoSlow event = new EventNoSlow(this.movementInput.moveForward,this.movementInput.moveStrafe);
            Harmony.getInstance().getEventBus().post(event);
            if (!noSlowEvent.isCancel() && !event.isCancel()) {
        ...
        if (this.isSprinting()) {
            boolean omnidirectional = Harmony.getInstance().getModuleManager().getAutoSprint().isOmnidirectional()
                    && (this.movementInput.moveForward != 0 || this.movementInput.moveStrafe != 0);
            boolean flag5 = !this.movementInput.isMovingForward() || !flag4;
            boolean flag6 = (flag5 && !omnidirectional) || this.collidedHorizontally || this.isInWater() && !this.canSwim();
    ...
    public void lookAt(Direction horizontalFacing, int x, int y, int z) {
    }
    ...
    public ResourceLocation getLocationSkin() {
        ...
        if (moduleManager != null) {
            AutoSkin autoSkin = moduleManager.getAutoSkin();
            if (autoSkin != null && autoSkin.isVisuals()) {
                ResourceLocation custom = autoSkin.getCustomSkinLocation();
                if (custom != null) {
                    return custom;
                }
            }
