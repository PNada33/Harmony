...
public abstract class AbstractBlock {
    ...
    public float getPlayerRelativeBlockHardness(BlockState state, PlayerEntity player, IBlockReader worldIn, BlockPos pos) {
        ...
        if (f == -1.0F) {
            ...
            AutoActions autoActions = Harmony.getInstance().getModuleManager().getAutoActions();

            int i = player.func_234569_d_(state) ? 30 : 100;
            return (autoActions.isState() && autoActions.actions.getValueByName("AutoTool").get() && autoActions.silent.get() && autoActions.itemIndex != -1 ? player.getDigSpeed(state, Minecraft.getInstance().player.inventory.getStackInSlot(autoActions.itemIndex), autoActions.itemIndex) : player.getDigSpeed(state)) / f / (float) i;
        }
    }
    ...
    public abstract static class AbstractBlockState extends StateHolder<Block, BlockState> implements ExtendedAbstractBlockState {
        ...
        public void cacheState() {
            ...
            this.isConditionallyFullOpaque = this.getBlock().isVariableOpacity() && this.isSolid && !this.transparent;
        ...
        {
            return this.getBlock().usesCustomShouldDrawFace(this.getSelf());
        }

        public Optional<Boolean> customShouldDrawFace(IBlockReader view, BlockState sideState, BlockPos thisPos, BlockPos sidePos, Direction side)
        {
            return this.getBlock().customShouldDrawFace(view, this.getSelf(), sideState, thisPos, sidePos, side);
        }

        public boolean shouldAttemptToCull()
        {
            return this.getBlock().shouldAttemptToCull(this.getSelf());
        }

        public boolean cantCullAgainst()
        {
            return this.getBlock().cantCullAgainst(this.getSelf());
        }
        ...
        public boolean causesSuffocation(IBlockReader worldIn, BlockPos pos) {
            ...
            if (antiPush.isState()
                    && antiPush.modes.getValueByName("Блоки").get()) {
                return false;
            }
