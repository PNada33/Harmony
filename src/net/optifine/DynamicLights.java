...
public class DynamicLights
{
    ...
    private static final double MAX_ENTITY_TRACK_DIST_SQ = 9216.0D;
    ...
    public static void update(WorldRenderer renderGlobal)
    {
        ...
        long updateIntervalMs = Config.isDynamicLightsFast() ? 500L : 100L;

        if (i >= timeUpdateMs + updateIntervalMs)
        {
            timeUpdateMs = i;
    ...
    private static void updateMapDynamicLights(WorldRenderer renderGlobal)
    {
        ...
        if (clientworld != null)
        {
            ...
            double viewX = viewEntity != null ? viewEntity.getPosX() : 0.0D;
            double viewY = viewEntity != null ? viewEntity.getPosY() : 0.0D;
            double viewZ = viewEntity != null ? viewEntity.getPosZ() : 0.0D;

            for (Entity entity : clientworld.getAllEntities())
            {
                if (viewEntity != null && entity != viewEntity)
                {
                    double dx = entity.getPosX() - viewX;
                    double dy = entity.getPosY() - viewY;
                    double dz = entity.getPosZ() - viewZ;

                    if (dx * dx + dy * dy + dz * dz > MAX_ENTITY_TRACK_DIST_SQ)
                    {
                        removeDynamicLight(entity, renderGlobal);
                        continue;
                    }
                }
    ...
    private static void removeDynamicLight(Entity entity, WorldRenderer renderGlobal)
    {
        DynamicLight dynamiclight = mapDynamicLights.remove(entity.getEntityId());
