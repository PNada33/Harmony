...
public class GlStateManager
{
    ...
    private static final BooleanState[] LIGHT_ENABLE = IntStream.range(0, 8).mapToObj((p_lambda$static$1_0_) ->
    {
        return new BooleanState(16384 + p_lambda$static$1_0_);
    }).toArray((p_lambda$static$2_0_) ->
    {
        return new BooleanState[p_lambda$static$2_0_];
    });
    ...
    private static final TextureState[] TEXTURES = IntStream.range(0, 32).mapToObj((p_lambda$static$3_0_) ->
    {
        return new TextureState();
    }).toArray((p_lambda$static$4_0_) ->
    {
        return new TextureState[p_lambda$static$4_0_];
    });
    ...
    private static boolean blendColorDirty = true;
    private static float blendRed;
    private static float blendGreen;
    private static float blendBlue;
    private static float blendAlpha;
    private static int blendEquation = Integer.MIN_VALUE;
    private static double clearDepth = Double.NaN;
    private static int clearStencil = Integer.MIN_VALUE;
    ...
    {
        if (blendColorDirty || red != blendRed || green != blendGreen || blue != blendBlue || alpha != blendAlpha)
        {
            blendColorDirty = false;
            blendRed = red;
            blendGreen = green;
            blendBlue = blue;
            blendAlpha = alpha;
    ...
    public static void blendEquation(int blendEquationIn)
    {
        RenderSystem.assertThread(RenderSystem::isOnRenderThread);

        if (blendEquationIn != blendEquation)
        {
            blendEquation = blendEquationIn;
            GL14.glBlendEquation(blendEquationIn);
        }
    }
    ...
    {
        RenderSystem.assertThread(RenderSystem::isOnRenderThread);
        TexGenCoord glstatemanager$texgencoord = getTexGen(texGen);
    ...
    public static void clearDepth(double depth)
    {
        ...
        if (Double.compare(depth, clearDepth) != 0)
        {
            clearDepth = depth;
