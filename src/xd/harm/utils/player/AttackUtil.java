...
public class AttackUtil implements IMinecraft {
    public static boolean isPlayerFalling(boolean onlyCrit, boolean onlySpace, boolean sync) {
        ...
        boolean reasonForAttack = mc.player.isOnLadder() 
        ...
        float attackStrength = mc.player.getCooledAttackStrength(sync ? Harmony.getInstance().getTpsCalc().getAdjustTicks() : 1.0f);

        if (attackStrength < 0.92) return false;

        if (!reasonForAttack && onlyCrit) {
