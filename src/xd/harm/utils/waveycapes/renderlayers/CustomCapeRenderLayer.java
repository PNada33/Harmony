package xd.harm.utils.waveycapes.renderlayers;

import xd.harm.utils.waveycapes.*;
import xd.harm.utils.waveycapes.math.Vector3;
...
public class CustomCapeRenderLayer extends LayerRenderer<AbstractClientPlayerEntity, PlayerModel<AbstractClientPlayerEntity>> {
    ...
    public void renderVanillaCape(MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn, AbstractClientPlayerEntity player, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        if (!player.isInvisible() && capeVisible(player)) {
    ...
    /**
     * Плащ Harmony (функция «Плащ» в Figura Cosmetic) не должен зависеть от
     * галочки «Плащ» в настройках скина: игрок её обычно выключает, и тогда
     * ванильная проверка isWearing(CAPE) гасила косметику целиком.
     */
    public static boolean capeVisible(AbstractClientPlayerEntity player) {
        if (player == null || !player.hasPlayerInfo() || player.getLocationCape() == null) {
            return false;
        }
        ...
        try {
            return xd.harm.utils.figura.CosmeticFeatures.isEnabled(xd.harm.utils.figura.CosmeticFeatures.RAINCOAT)
                    && player instanceof net.minecraft.client.entity.player.ClientPlayerEntity;
    ...
    private CapeRenderer getCapeRenderer(AbstractClientPlayerEntity abstractClientPlayer, IRenderTypeBuffer multiBufferSource) {
        if (!capeVisible(abstractClientPlayer)) {
            return null;
        }
