...
public class CowBrain implements IMinecraft {
    ...
    private Vector3d resolveDesiredPosition(Vector3d playerPos, LivingEntity target) {
        if (target != null && entity instanceof ClientPlayerEntity) {
            ...
            Vector3d aroundTarget = playerPos.subtract(targetPos);
            if (aroundTarget.lengthSquared() < 1.0E-4D) {
                aroundTarget = new Vector3d(0.8D, 0.0D, 0.8D);
            } else {
                aroundTarget = aroundTarget.normalize().scale(1.35D);
            }
            return findGroundedPosition(targetPos.add(aroundTarget.x, 0.0D, aroundTarget.z));
        }
    ...
    private Vector3d updateHorizontalMotion(Vector3d currentPos, Vector3d desiredPos, LivingEntity target) {
        ...
        if (target != null && entity instanceof ClientPlayerEntity) {
            ...
            if (box.maxX > targetBox.minX
                ...
                motion = motion.mul(-0.4D, 1.0D, -0.4D);
            }
        }
        return motion;
    }
