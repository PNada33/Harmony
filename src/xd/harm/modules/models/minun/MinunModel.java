package xd.harm.modules.models.minun;
...
public class MinunModel implements IMinecraft {

    private static final ResourceLocation MODEL_LOCATION = new ResourceLocation("minecraft", "harmony/models/pet/minun_mesh.json");
    private static final ResourceLocation TEXTURE_LOCATION = new ResourceLocation("minecraft", "harmony/images/pet/minun.png");
    ...
    public void render(MatrixStack matrixStack, EventRender3D event, MinunBrain brain) {
        ensureLoaded();
        ...
        matrixStack.push();
        matrixStack.translate(sideShift, bob - 0.01 * 19f, 0.0D);
    ...
    private void renderNode(MatrixStack matrixStack, int nodeIndex, AnimationPose pose) {
        ...
        if ("Leg1".equals(node.name)) {
            renderCopiedLeftLeg(matrixStack, pose);
            return;
        }
    ...
    private void renderCopiedLeftLeg(MatrixStack matrixStack, AnimationPose pose) {
        int leftLegIndex = findNodeIndex("Leg0");
        if (leftLegIndex < 0 || leftLegIndex >= nodes.size()) {
            return;
        }

        ModelNode leftLeg = nodes.get(leftLegIndex);
        matrixStack.push();
        matrixStack.translate(-leftLeg.translation[0] + 0.5f, leftLeg.translation[1], leftLeg.translation[2]);
        matrixStack.rotate(new Quaternion(leftLeg.rotation[0], leftLeg.rotation[1], leftLeg.rotation[2], leftLeg.rotation[3]));
        if (!isUnitScale(leftLeg.scale)) {
            matrixStack.scale(leftLeg.scale[0], leftLeg.scale[1], leftLeg.scale[2]);
        }

        applyNodeAnimation(matrixStack, leftLeg.name, pose);

        if (!leftLeg.triangles.isEmpty()) {
            renderTriangles(matrixStack, leftLeg.triangles);
        }

        for (int childIndex : leftLeg.children) {
            renderNode(matrixStack, childIndex, pose);
        }
        matrixStack.pop();
    }
    ...
    private void applyNodeAnimation(MatrixStack matrixStack, String nodeName, AnimationPose pose) {
        if (applyClipAnimation(matrixStack, nodeName, pose)) {
            return;
        }

        if ("Body".equals(nodeName)) {
            float breathe = (float) Math.sin(pose.idleCycle * 0.24F) * 0.9F * pose.idleBlend;
            matrixStack.translate(0.0D, (float) Math.sin(pose.idleCycle * 0.24F) * 0.003F * pose.idleBlend, 0.0D);
            matrixStack.rotate(Vector3f.ZP.rotationDegrees(pose.shoulderSway * 0.14F));
            matrixStack.rotate(Vector3f.XP.rotationDegrees(breathe + pose.walkBlend * 1.2F));
            return;
        }

        if ("Head".equals(nodeName)) {
            float idleYaw = (float) Math.sin(pose.idleCycle * 0.32F + 0.4F) * 1.8F * pose.idleBlend;
            float idlePitch = (float) Math.cos(pose.idleCycle * 0.28F + 0.7F) * 1.2F * pose.idleBlend;
            matrixStack.rotate(new Quaternion(0.00851F, -0.194905F, 0.042781F, 0.979852F));
            matrixStack.rotate(Vector3f.YP.rotationDegrees(-pose.headYaw * 0.75F + idleYaw));
            matrixStack.rotate(Vector3f.XP.rotationDegrees(pose.headPitch * 0.55F + idlePitch));
            return;
        }

        if ("Tail".equals(nodeName)) {
            matrixStack.rotate(Vector3f.YP.rotationDegrees(pose.tailBaseYaw * 1.45F));
            matrixStack.rotate(Vector3f.XP.rotationDegrees(pose.tailBaseRoll * 0.35F));
            return;
        }

        if ("Leg0".equals(nodeName)) {
            float lift = Math.max(0.0F, -pose.rearLeftSwing) * 0.0024F;
            matrixStack.translate(0.0D, lift, 0.0D);
            matrixStack.rotate(Vector3f.XP.rotationDegrees(pose.rearLeftSwing * 0.72F));
            matrixStack.rotate(Vector3f.ZP.rotationDegrees(-pose.hipSway * 0.18F));
            return;
        }
        if ("Leg1".equals(nodeName)) {
            float lift = Math.max(0.0F, -pose.rearRightSwing) * 0.0024F;
            matrixStack.translate(0.0D, lift, 0.0D);
            matrixStack.rotate(Vector3f.XP.rotationDegrees(pose.rearRightSwing * 0.72F));
            matrixStack.rotate(Vector3f.ZP.rotationDegrees(pose.hipSway * 0.18F));
            return;
        }
        if ("Arm0".equals(nodeName)) {
            matrixStack.rotate(Vector3f.XP.rotationDegrees(pose.frontLeftSwing * 0.46F));
            matrixStack.rotate(Vector3f.ZP.rotationDegrees(-8.0F + pose.earFlop + pose.shoulderSway * 0.35F));
            return;
        }
        if ("Arm1".equals(nodeName)) {
            matrixStack.rotate(Vector3f.XP.rotationDegrees(pose.frontRightSwing * 0.46F));
            matrixStack.rotate(Vector3f.ZP.rotationDegrees(8.0F - pose.earFlop - pose.shoulderSway * 0.35F));
            return;
        }
        if ("Ear0".equals(nodeName)) {
            matrixStack.rotate(Vector3f.ZP.rotationDegrees(-pose.earFlop * 0.45F));
            return;
        }
        if ("Ear".equals(nodeName)) {
            matrixStack.rotate(Vector3f.ZP.rotationDegrees(pose.earFlop * 0.45F));
        }
    }
    ...
    private void ensureLoaded() {
        ...
        try (InputStream stream = Minecraft.getInstance().getResourceManager().getResource(MODEL_LOCATION).getInputStream();
            ...
            failed = true;
            System.err.println("Failed to load minun pet model from " + MODEL_LOCATION);
            e.printStackTrace();
        }
    }
    ...
    private float resolveModelYawOffset() {
        int headIndex = findNodeIndex("Head");
        int tailIndex = findNodeIndex("Tail");
