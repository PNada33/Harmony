...
public class PenguinModel implements IMinecraft {

    private static final ResourceLocation MODEL_LOCATION = new ResourceLocation("minecraft", "harmony/models/pet/penguin_mesh.json");
    private static final ResourceLocation TEXTURE_LOCATION = new ResourceLocation("minecraft", "harmony/images/pet/penguin.png");
    ...
    public void render(MatrixStack matrixStack, EventRender3D event, PenguinBrain brain) {
        ensureLoaded();
    ...
    private void renderNode(MatrixStack matrixStack, int nodeIndex, AnimationPose pose) {
        ...
        applyNodeAnimation(matrixStack, node, pose);
    ...
    private void applyNodeAnimation(MatrixStack matrixStack, ModelNode node, AnimationPose pose) {
        if (!shouldAnimateNode(node)) {
            return;
        }

        String nodeName = node.name;
        if ("Head".equals(nodeName) || "head".equals(nodeName)) {
        ...
        if ("Body".equals(nodeName) || "BodyUpper".equals(nodeName) || "body".equals(nodeName)) {
        ...
        if ("left_arm".equals(nodeName)) {
            matrixStack.rotate(Vector3f.ZP.rotationDegrees(-22.0F + pose.earFlop * 0.35F + pose.shoulderSway * 0.12F));
    ...
    private boolean shouldAnimateNode(ModelNode node) {
        return shouldAnimateNode(node.name, node.children.length > 0);
    }

    private boolean shouldAnimateNode(String nodeName, boolean hasChildren) {
        if (hasChildren || nodeName == null || nodeName.isEmpty()) {
            return true;
        }

        for (ModelNode other : nodes) {
            if (nodeName.equals(other.name) && other.children.length > 0) {
                return false;
            }
        }
        return true;
    ...
    private void ensureLoaded() {
        ...
        try (InputStream stream = Minecraft.getInstance().getResourceManager().getResource(MODEL_LOCATION).getInputStream();
            ...
            failed = true;
            System.err.println("Failed to load penguin pet model from " + MODEL_LOCATION);
            e.printStackTrace();
        }
    }
        ...
        if (animationArray != null) {
            for (int i = 0; i < animationArray.size(); i++) {
                ...
                if ("idle".equalsIgnoreCase(animation.name)
                        || (idleAnimation == null && animation.name.toLowerCase().contains("pose"))) {
