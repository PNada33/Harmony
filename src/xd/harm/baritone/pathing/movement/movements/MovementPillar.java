...
package xd.harm.baritone.pathing.movement.movements;
...
import xd.harm.baritone.api.utils.VecUtils;
...
import xd.harm.baritone.pathing.movement.MovementState;
...
public class MovementPillar extends Movement {
    ...
    public MovementState updateState(MovementState state) {
        ...
        Rotation rotation = RotationUtils.calcRotationFromVec3d(ctx.playerHead(),
                ...
                new Rotation(ctx.player().rotationYawHead, ctx.player().rotationPitchHead));
        if (!ladder) {
            state.setTarget(new MovementState.MovementTarget(new Rotation(ctx.player().rotationYawHead, rotation.getPitch()), true));
        }
