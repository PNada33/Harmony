...
public class VClipCommand implements Command, MultiNamedCommand {
    final Prefix prefix;
    final Logger logger;
    final Minecraft mc;
    ...
    public void execute(Parameters parameters) {
        ...
        String direction = parameters.asString(0)
                .orElseThrow(() -> new CommandException(TextFormatting.RED + "× " + TextFormatting.RED + "Укажите направление или дистанцию"));
        switch (direction) {
        ...
        if (yOffset != 0) {
            ...
            if (hasElytra) switchElytra(elytraSlot, true);
            teleport(yOffset, hasElytra);
            if (hasElytra) switchElytra(elytraSlot, false);
        } else {
            logger.log(TextFormatting.DARK_GRAY + "" + TextFormatting.RED + "× " + TextFormatting.WHITE + "Телепортация невозможна");
        }
    }
    ...
    private float findOffset(BlockPos playerPos, boolean toUp) {
        ...
        for (int i = startY; i != endY; i += step) {
            ...
            if (mc.world.getBlockState(targetPos) == Blocks.BEDROCK.getDefaultState() && !toUp) {
                logger.log(TextFormatting.DARK_GRAY + "" + TextFormatting.RED + "× " + TextFormatting.WHITE + "Бедрок блокирует путь");
                return 0;
            }
        }
        return 0;
    ...
    private void teleport(float yOffset, boolean elytra) {
        if (elytra) {
            ...
            mc.player.connection.sendPacket(new CPlayerPacket.PositionPacket(mc.player.getPosX(), mc.player.getPosY() + yOffset, mc.player.getPosZ(), false));
            mc.player.connection.sendPacket(new CEntityActionPacket(mc.player, CEntityActionPacket.Action.START_FALL_FLYING));
            mc.player.setPosition(mc.player.getPosX(), mc.player.getPosY() + yOffset, mc.player.getPosZ());
            logger.log(TextFormatting.DARK_GRAY + "" + TextFormatting.GREEN + "⟵⟶ " + TextFormatting.WHITE + "Elytra-clip: " + String.format("%.1f", yOffset) + TextFormatting.GRAY + " блоков");
            return;
        }
        ...
        mc.player.connection.sendPacket(new CPlayerPacket.PositionPacket(mc.player.getPosX(), mc.player.getPosY() + yOffset, mc.player.getPosZ(), false));
        mc.player.setPosition(mc.player.getPosX(), mc.player.getPosY() + yOffset, mc.player.getPosZ());
        String arrow = yOffset > 0 ? "↑" : "↓";
        logger.log(TextFormatting.DARK_GRAY + "" + TextFormatting.GREEN + arrow + " " + TextFormatting.WHITE + "Телепорт: " + String.format("%.1f", Math.abs(yOffset)) + TextFormatting.GRAY + " блоков");
    }

    private float parseOffset(String distance) {
        if (NumberUtils.isNumber(distance)) return Float.parseFloat(distance);
        logger.log(TextFormatting.DARK_GRAY + "" + TextFormatting.RED + "× " + TextFormatting.WHITE + "Некорректное число");
        return 0;
    }
