...
public abstract class StarLightEngine {
    ...
    protected final void setupCaches(final IChunkLightProvider chunkProvider, final int centerX, final int centerY, final int centerZ,
        ...
        for (int dz = -radius; dz <= radius; ++dz) {
            for (int dx = -radius; dx <= radius; ++dx) {
                ...
                final IChunk chunk = (IChunk)chunkProvider.getChunkForLight(cx, cz);
    ...
    protected static BlockState getBlockStateFromLocalIndex(final ChunkSection section, final int localIndex) {
        return section.getBlockState(localIndex & 15, (localIndex >>> 8) & 15, (localIndex >>> 4) & 15);
    }
    ...
    protected final BlockState getBlockState(final int sectionIndex, final int localIndex) {
        ...
        if (section != null) {
            return section == EMPTY_CHUNK_SECTION ? AIR_BLOCK_STATE : getBlockStateFromLocalIndex(section, localIndex);
        }
